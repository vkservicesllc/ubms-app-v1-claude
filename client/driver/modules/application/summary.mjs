...
$.ajax(`/api/resource/application/${formId}/accidents`, {
    success(response) {
        ...
        const accidents = $.ajax('/api/public/enum/driver-application?filter=accidents', {
            method: 'POST',
            async: false,
        }).responseJSON;
        let html = '';
        ...
        data.forEach((row, i) => {
            ...
            if (other) type = other;
                ...
                accidentLoop: for (const group in accidents) {
    ...
    success(response) {
        ...
        if (length) {
            ...
            data.forEach((row, i) => {
                ...
                let subject = '';
                if (row.fmcsr !== null)
                    subject += `<small>FMCSR —</small> ${row.fmcsr ? 'Yes' : 'No'}`;
                ...
                if (row.gapExpl)
                    html += `<tr><th class="text-secondary">Employment Gap:</th><td>${row.gapExpl}</td></tr>`;
                html += '</tbody>';
            });
