        ...
        onComplete(mask, $since) {
            {
                ...
                if (!since.isValid()) {
                    ...
                    const maxDate = idx
                        ? moment(
                              $(`${selector.id.text.prevAddrSince}-${idx - 1}`).val(),
                              'MM/DD/YYYY',
                          )
                    ...
                    if (since.isSameOrAfter(maxDate)) {
                        ...
                        if (nextSince && since.isSameOrBefore(nextSince)) {
                            ...
                            const $enough = $since
                                ...
                                .parent()
                                .parent()
                                .find('.driver-application-prev-addr-enough-hidden-input');
                            const minDate = moment($(selector.id.hidden.appliedOn).val())
                                .clone()
                                .subtract(3, 'years');
