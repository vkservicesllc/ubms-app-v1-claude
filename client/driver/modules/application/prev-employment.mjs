...
inputEvent(employedId.no, {
    onChange(value, $el) {
        if (selected === true) {
            if (
                confirm(
                    'By confirming, you acknowledge that your pre-employment data will be erased!',
                )
            ) {
                $section.hide();
                $emplList.html(null);
                    ...
                    if (end && start.isAfter(end)) {
                        ...
                        if (expStartDate) {
                            ...
                            if (diff >= 3 && start.isBefore(expStart)) {
                                $start.addClass('is-invalid');
                                $help.html(
                                    `* Started before reported <b>${expStart.format('ll').replaceAll(' ', '&nbsp;')}</b>`,
                                );
            ...
            employers.map((employment) => {
                ...
                if (date.current.isSameOrBefore(date.previous)) {
                    ...
                    if (difference > 30) {
                        ...
                        list +=
                            '<li class="list-group-item"><div class="d-flex flex-column flex-md-row justify-content-between">';
                        ...
                        list += `<textarea class="form-control explain-employment-gap${fieldCls}" name="explGap[]" placeholder="Provide explanation here..." data-id="${_id}" required>${gapExpl || ''}`;
...
function closeForm() {
    ...
    $submit.employer
        .removeClass('btn-primary bg-primary-subtle btn-success bg-success-subtle')
        .text(null);
    // $btnContainer.employment.show()
