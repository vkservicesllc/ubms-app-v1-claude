        ...
        if (since) {
            ...
            if (!since.isValid()) {
                ...
                if (since.isAfter(today)) {
                    ...
                    const limit = moment($(selector.id.hidden.appliedOn).val())
                        .clone()
                        .subtract(3, 'years');
                    ...
                    if (since.isBefore(limit)) {
                        ...
                        props.forEach(
                            (prop) => (livedAbroad[prop] = $(livedAbroadId[prop]).prop('checked')),
                        );
                        ...
                        if (livedAbroad.no) {
                            $('#address-list')
                                ...
                                .each(function (i) {
                                    ...
                                    if (visible) {
                                        $livedAbroad.no = $(this).find(
                                            TR.livedAbroad2.no + `-${i}`,
                                        );
                                        props.forEach((prop) =>
                                            $livedAbroad[prop].prop('disabled', false),
                                        );
                                    ...
                                    if (!i)
                                        minDate = $(this)
                                            .find(selector.class.text.prevAddrSince)
                                            .val();
                                });
                        }
