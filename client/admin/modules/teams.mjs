    ...
    $.ajax({
        ...
        success(response) {
            ...
            for (const [idx, row] of data.entries()) {
                ...
                if (scoped)
                    html +=
                        '<sup style="margin-left: .35rem; font-size: .4em;"><i class="fas fa-star has-text-warning-45" title="Scoped users only"></i></sup>';
                html += '</p>';
                if (description)
                    html += `<p class="subtitle has-text-primary-30 mb-3" style="font-size: .95em;">${escapeHTML(description)}</p>`;
                ...
                $.ajax({
                    ...
                    success(response) {
                        ...
                        if (target == 'edit') {
                            ...
                            $title.upsert.html(
                                `<small>Modify Team</small> <strong>${escapeHTML(name)}</strong>`,
                            );
                            setTip.passed('name');
                            ...
                            $title.profile.html(
                                `<strong>${escapeHTML(name)}</strong> <small>Profile</small>`,
                            );
                            ...
                            list +=
                                '<nav class="breadcrumb is-centered"><ul><li><a>Carrier</a></li><li><a>Driver Applications</a></li></ul></nav>';
                            ...
                            $settings.html(list);
                            $title.settings.html(
                                `<strong>${escapeHTML(name)}</strong> <small>Settings</small>`,
                            );
            ...
            $('.team-relationship').on('click', function () {
                ...
                $.ajax({
                    ...
                    success(response) {
                        ...
                        const { _id, name } = team;
                        $title.relationship.html(
                            `<small>Assign ${capitalizeFirst(relType)} to</small> <strong>${escapeHTML(name)}</strong>`,
                        );
                        data.all = data.all.filter((item) => !item.unscoped);

                        if (!data.all.length) {
                            $relationship.html(
                                `<i class="has-text-danger-65">No ${relType} to assign</i>`,
                            );
                        ...
                        data.all.map((item) => {
                            if (relType === 'users')
                                item.name =
                                    new Person(item).fullName('AL') +
                                    ` <small>(${item.email}) - ${item.expansion.status} in ${item.expansion.location}</small>`;
