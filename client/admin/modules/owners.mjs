    ...
    for (const catId in categories) {
        ...
        columns.push({
            ...
            render(data, type) {
                ...
                return type == 'display'
                    ? `<${tag} class="tag is-${style}">${count}</${tag}>`
                    : count;
            },
        });
    }
...
columns.push({
    ...
    render(data, type, row) {
        ...
        cell += `<a class="has-text-primary-35 modify-owner-address" data-id="${row._id}" title="Add/Modify Address"><i class="fas fa-house-chimney-window"></i></a>`;
        ...
        cell += `<a class="has-text-success-45 modify-owner" data-id="${row._id}" title="Modify/Update"><i class="fas fa-pen-to-square"></i></a>`;
        cell += '</div>';
...
const table = $('#owners-table').DataTable({
    ...
    language: {
        emptyTable:
            '<span class="has-text-danger">No company owners registered at this time</span>',
    },
...
onDraw(table, () => {
    if (!refreshed) $('.modal-cancel, .modal-close, .delete').click(closeModals);
