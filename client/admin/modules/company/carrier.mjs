...
const tip = {
    ...
    failedForm:
        '<i class="fas fa-close"></i>&nbsp; Credentials can not have dublicates<br /><i class="fas fa-close"></i>&nbsp; Data can not be submitted',
};
...
for (const prop in TS) {
    ...
    if (number) {
        ...
        $tip.addClass('is-success').html(tip.success).show();
    }
}

let credClass = [];
for (const prop of ['alpha', 'alphaNumber', 'number', 'permit']) credClass.push(TCS[prop]);
        ...
        if (number) {
            ...
            $.ajax('/api/unique/carrier', {
                ...
                success(response) {
                    ...
                    } else $tip.addClass('is-danger').html(tip.failed).show();
                },
            });
        }
    },
...
$form.submit(function (event) {
    ...
    if (!formValid()) $formTip.html(tip.failedForm);
...
function formValid() {
    ...
    $('.carrier-tip').each(function () {
        ...
        if ($(this).hasClass('is-danger')) valid = false;
    });
