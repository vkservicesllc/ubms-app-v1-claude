...
export const closeModals = () => {
    ...
    if (!$gender.find('option[value=""]').length) $gender.prepend('<option value="">--</option>');
};
...
export const openModifyModal = (_id) => {
    ...
    $.ajax(url + _id, {
        success(response) {
            ...
            $title.form.html(
                `<small class="has-text-grey is-size-6">Modify Owner</small> <strong>${escapeHTML(new Person(data).fullName())}</strong>`,
            );
            $(id).val(_id);
...
export const openModifyPhoneModal = (_id) => {
    ...
    $.ajax(url + _id, {
        success(response) {
            ...
            $title.phone.html(
                `<small class="has-text-grey is-size-6">Modify Phone for Owner</small> <strong>${escapeHTML(new Person(data).fullName())}</strong>`,
            );
            $(modifyId).val(_id);
...
export const openDeleteModal = (_id) => {
    ...
    $.ajax(url + _id, {
        success(response) {
            ...
            $title.delete.html(
                `<small class="has-text-danger is-size-6">Delete Owner</small> <strong>${escapeHTML(new Person(data).fullName())}</strong>`,
            );
            $(deleteId).val(_id);
