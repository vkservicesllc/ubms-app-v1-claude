...
import {
  ...
  einEvent,
  dunsEvent,
} from '/modules/events/company.mjs';
...
const sinceId = TS.since,
  effectiveId = TS.effective;
const einId = TS.ein,
  dunsId = TS.duns,
  websiteId = TS.website;
...
const tipDefs = {};
for (const key in $tip) tipDefs[key] = $tip[key].html();
...
const openUpsertModal = (target, action = 'insert', data, since) => {
  ...
  switch (action) {
    case 'update':
      ...
      $effective.val(
        data.since !== '0000-00-00' ? moment(data.since).format('MM/DD/YYYY') : 'Launch Date',
      );
...
const closeUpsertModal = () => {
  ...
  if (!$(coTypeId).find('option[value=""]').length)
    $(coTypeId).prepend('<option value="">--</option>');
  if (!$(stateId).find('option[value=""]').length)
    $(stateId).prepend('<option value="">--</option>');
  if (!$(mailStateId).find('option[value=""]').length)
    $(mailStateId).prepend('<option value="">--</option>');
  $title.html(null);
...
$('#upsert-form').submit(function (evt) {
  ...
  const data = {},
    target = $target.upsert.val(),
    since = $effectiveMatch.upsert.val();
  let method = 'POST',
    url = `/api/resource/companies/${_id}/${target}`;
  ...
  $.ajax({
    ...
    error() {
      ...
      $error.upsert
        .html(
          '<i class="fa fa-exclamation-triangle"></i> Failed to write data: Effective date may exist',
        )
        .show();
    },
  });
...
$.ajax(`/api/resource/companies/${_id}/history`, {
  success(response) {
    ...
    const defs = {
      a: (span) =>
        `<tr><td class="has-text-centered has-text-danger-65" colspan="${span}"><small><i>`,
      b: '</i></small></td></tr>',
      ...
      aAttr: {
        edit: (row, target, value) =>
          `class="edit-event" title="Edit selected ${value}" data-target="${target}" data-since="${row.since}" href=""`,
        delete: (row, target, value) =>
          `class="delete-event ml-2" title="Delete selected ${value}" data-target="${target}" data-since="${row.since}" href=""`,
      },
    };
    ...
    const setDate = (row) =>
      (row.since !== '0000-00-00'
        ? moment(row.since).format('ll')
        : '<em class="has-text-danger has-text-weight-normal">Launch Date</em>') +
    ...
    names.map((row, i) => {
      ...
      if (!row.initial)
        list.names += `<a ${defs.aAttr.delete(row, 'names', 'name')}><i class="fa fa-trash has-text-danger-60"></i></a>`;
      list.names += '</td></tr>';
    });
    ...
    ownerships.map((row, i) => {
      ...
      if (!row.initial)
        list.ownerships += `<a ${defs.aAttr.delete(row, 'ownerships', 'ownership')}><i class="fa fa-trash has-text-danger-60"></i></a>`;
    ...
    phones.map((row, i) => {
      ...
      if (!row.initial)
        list.phones += `<a ${defs.aAttr.delete(row, 'phones', 'phone')}><i class="fa fa-trash has-text-danger-60"></i></a>`;
      list.phones += '</td></tr>';
    });
    ...
    addresses.map((row, i) => {
      const mailSup = row.mail
        ? ' &nbsp;<sup class="has-text-info-45 enforced"><i class="fa fa-envelope"></i></sup>'
        : '';
      ...
      if (!row.initial)
        list.addresses += `<a ${defs.aAttr.delete(row, 'addresses', 'address')}><i class="fa fa-trash has-text-danger-60"></i></a>`;
    ...
    inputEvent(effectiveId, {
      datepicker: {
        minDate: since !== '0000-00-00' ? moment(since, 'YYYY-MM-DD').toDate() : undefined,
        maxDate: 0,
      },
      onChange(date, $date) {
