...
if (_id) {
    ...
    const relationships = $.ajax(`/api/resource/users/${_id}/relationships`, { async: false })
        .responseJSON.data;
...

    {
        ...
        const options = { ...defOpts },
            optgroups = {};

        for (const prop of keys) {
            optgroups[prop] = roles[prop].reduce(
                (cat, { _id, name, location, category, expansion }) => {
...

    {
        ...
        for (const prop of keys) {
            optgroups[prop] = companies[prop].reduce(
                (cat, { _id, name, category, active, until }) => {
                    if (!cat[category]) cat[category] = [];
