    ...
    const table = new DataTable('#companies-table', {
        ajax: {
            ...
            dataSrc(response) {
                ...
                owners = Array.from(new Set(owners.map((owner) => JSON.stringify(owner)))).map(
                    (str) => JSON.parse(str),
                );
        ...
        columns: [
...

            {
                ...
                render(data, type, row) {
                    ...
                    if (!row.confirmed)
                        return '<i class="fa fa-hourglass-half has-text-primary"></i>';
        ...
        initComplete(settings, response) {
            ...
            owners.map(
                (owner) =>
                    (dropdown.owners += `<option value="${owner._id}">${owner.name}</option>`),
            );
            states.map(
                (state) =>
                    (dropdown.states += `<option value="${state.code}">${state.name}</option>`),
            );
