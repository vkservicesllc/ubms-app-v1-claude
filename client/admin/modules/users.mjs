        ...
        columns: [
            {
                ...
                render(data, type, row) {
                    if (
                        row.self ||
                        row.status === 'D' ||
                        (adminStatus === 'A' && row.username && row.DS)
                    )
                        return '<i class="fas fa-lock has-text-grey"></i>';
                    if (row.events.declinedAt)
                        return '<i class="fas fa-user-times has-text-dark"></i>';
...

            {
                ...
                render(data, type, row) {
                    ...
                    if (!lastLogin)
                        return '<small class="has-text-grey"><i>Never logged in</i></small>';
    ...
    onDraw(table, () => {
        if (!refreshed) {
            const closeModals = () => {
                ...
                $location
                    .prop('disabled', false)
                    .val(adminLocation !== 'US' ? adminLocation : null);
                        ...
                        success(response) {
                            ...
                            if (updateLog)
                                updateLog.forEach((log) => {
                                    const { data, oldData, modifiedBy, modifiedIn, modifiedAt } =
                                        log;
                                    const updates = [];
