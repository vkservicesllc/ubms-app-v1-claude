...
loginEvent({
  ...
  onAjax(response, params) {
    ...
    if (!username) $error.username.text(error.username || 'User error').show();
    if (!password) $error.password.text(error.password || 'Password error').show();
    if (condition != 'A') $error.username.text(error.username || 'Condition error').show();
