...
(() => {
  ...
  $calendar.expiresOn.calendar({
    ...calSettings,
    minDate: moment(finishedAt).add(1, 'days').toDate(),
  });
  if (expiresOn) $calendar.expiresOn.calendar('set date', new Date(moment(expiresOn).toDate()));

  $calendar.issuedOn.calendar({
    ...calSettings,
    maxDate: moment(finishedAt).toDate(),
  });
  if (issuedOn) $calendar.issuedOn.calendar('set date', new Date(moment(issuedOn).toDate()));
  ...
  } else $fields.find('input').prop('disabled', false);
