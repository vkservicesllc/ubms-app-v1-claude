...
(() => {
    ...
    $modal.add.modal({
        ...
        onHidden() {
            $form.add
                .find('input:not([name="_aplId[]"]):not([type="checkbox"]), textarea')
                .val(null);
            $form.add.find('[type="checkbox"]').prop('checked', false);
    ...
    $.ajax(`/api/resource/drivers/applications/${_id}/employments`, {
        success(response) {
            ...
            data.forEach((record, i) => {
                ...
                const $card = $(
                    '<div class="ui card" style="width: 100%;"><div class="content"></div></div>',
                );
                $card
                    .find('.content')
                    .html(
                        `<div class="header" style="font-size: 1.5rem;">Previous Employer <small># ${len--}</small>${tag}</div>`,
                    );
                ...
                $footer.append(
                    `<button class="ui red circular delete button" data-id="${_id}">Delete</button>`,
                );
                $footer.append(
                    `<button type="submit" form="${formId}" class="ui right floated dark green circular button" disabled>Save</button>`,
                );
                $footer.append(
                    '<span class="right floated unsaved-changes" style="display: none; margin-right: .71rem;"><i class="red exclamation triangle icon"></i></span>',
                );
                ...
                $form
                    .find(TS.endDate)
                    .removeAttr('id')
                    .val(leftOn ? moment(leftOn).format('ll') : null);
            ...
            zipEvent(TS.emplAddrZip, {
                onChange(zip, $zip, city, state) {
                    if (city && state) {
                        ...
                        const $stateDropdown = $zip
                            .parent()
                            .parent()
                            .find('.empl-addr-state-dropdown');
            ...
            $('.delete.button').on('click', function () {
                ...
                $.ajax(`/api/drivers/applications/prev-employer/${_id}`, {
                    ...
                    success(response) {
                        ...
                        let html =
                            '<p>Are you sure you would like to delete the following employer?</p>';
