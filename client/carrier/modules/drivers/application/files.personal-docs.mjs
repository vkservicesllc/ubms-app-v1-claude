...
(() => {
    ...
    const steps = {
        upload: {
            ...
            mec: ['Editor: Medical Certificate', 'Data Verification and Confirmation'],
            ssc: ['Editor: Social Security Card', 'Data Verification and Confirmation'],
        },
    };
    const activeStep = {
        dl: 0,
    ...
    const $dropdown = {
        ...
        dlChooseAddr: [
            $('#dl-choose-addr-confirm-dropdown'),
            null,
            function (value) {
                let since = null,
                    address1 = null,
    ...
    $calendar.dlExpiresOn2.calendar({
        ...calSettings,
        minDate: moment(finishedAt).add(1, 'days').toDate(),
    });
    ...
    nameEvent(TS.dlLastName, {
        ...
        onChange(lastName, $lastName, suffix) {
            if (suffix) $dropdown.dlSuffix[0].dropdown('set selected', suffix);
        },
    });
    ...
    $calendar.mecExpiresOn.calendar({
        ...calSettings,
        minDate: moment(finishedAt).add(1, 'days').toDate(),
    });
    if (mec?.expiresOn)
    ...
    if (legalDoc) {
        $calendar.legExpiresOn.calendar({
            ...calSettings,
