            ...
            data.map(employment => {
                ...
                if (date.current.isSameOrBefore(date.previous)) {
                    ...
                    if (difference > 30) {
                        ...
                        body[i].append(`<td><div class="field"><textarea class="explain-employment-gap" name="explGap[]" placeholder=" " rows="2" required>${gapExpl}</textarea></div></td>`)
                        body[i].append(`<td class="right aligned"><button class="ui tiny ${style} icon button"><i class="save icon"></i></button></td>`)
                        i++
                    }
                }
            })
