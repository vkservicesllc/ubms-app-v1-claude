...
const table = $('#driver-apl-prev-employers-table').DataTable({
    ajax: {
        ...
        dataSrc(response) {
            ...
            data.forEach((row) => {
                ...
                row.group +=
                    new Person(row.application).fullName() +
                    ` <small><small class="bull">•</small> ${formatTel(phone)}`;
    ...
    language: {
        emptyTable:
            '<span class="ui red text">No applicants have previous employers at this time</span>',
    },
