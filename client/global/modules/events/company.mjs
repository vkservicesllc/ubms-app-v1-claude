...
const categories = $.ajax('/api/public/enum/company?filter=categories', {
    async: false,
    method: 'POST',
}).responseJSON;
...
export const aliasEvent = (id, options = {}) => {
    ...
    inputEvent(id, {
        ...
        word: true,
        value,
        onInput(alias, $alias, pos) {
            alias = alias.replace(/[^A-Z]/, '');

            $alias.val(alias);
            if (onInput) onInput(alias, $alias, pos);
        },
        onChange,
        onFocus,
        onBlur,
