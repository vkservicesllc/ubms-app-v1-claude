...
const formSelectOptions = (data = {}, options = {}) => {
  ...
  if (emptyOpt !== null) html = nl + tabs + `<option value="">${emptyOpt}</option>`;
...
export const formSelect = (props = {}, data = {}, options = {}) => {
  const { id, name, value, size, multiple, required, disabled } = props;
  let { emptyOpt } = props;
  const classes = initializeClass(props);
  ...
  const nl = tabs > -1 ? `\n${'\t'.repeat(tabs)}` : '';
  const attrs =
    classAttr + idAttr + nameAttr + sizeAttr + multipleAttr + requiredAttr + disabledAttr;
...
const formCheck = (type, props = {}) => {
  ...
  for (const key in props) if (exclude.includes(key)) delete props[key];
