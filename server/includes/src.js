module.exports = {
  ...
  css(href) {
    if (href.substr(-4) != '.css' && href.substr(-1) != '^') href += '.css';
  ...
  render(instr = {}) {
    ...
    for (let css of instr.internal.css) html.css += this.css(css);
    for (let js of instr.internal.js) html.js += this.js(js);
    for (let mjs of instr.internal.mjs) html.mjs += this.mjs(mjs);
      ...
      if (set[type])
        switch (typeof set[type]) {
          case 'string':
            ...
            break;
          default:
            if (Array.isArray(set[type])) for (let chunk of set[type]) html[type] += `\n\t${chunk}`;
        }
    }
