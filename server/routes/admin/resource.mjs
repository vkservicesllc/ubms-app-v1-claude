...
router.post(
    '/update/company/:_id/static',
    User.mw.verify,
    User.mw.superAdminOnly,
    CompanyForm.validate('static'),
    validationCheck,
    async (req, res) => {
        try {
            const { _id } = req.params;
...
router.post(
    '/update/company/:_id/:action/:step',
    User.mw.verify,
    User.mw.superAdminOnly,
    dynamicValidator.companies,
    validationCheck,
    async (req, res) => {
...
router.post(
    '/update/company-owner/add/name',
    User.mw.verify,
    User.mw.superAdminOnly,
    OwnerForm.validate('name'),
    validationCheck,
    async (req, res) => {
        try {
            const { company: _companyId } = req.query;
...
router.post('/delete/:src/:_id?', User.mw.verify, async (req, res) => {
    try {
        ...
        if (src !== 'user' && res.session.user.status === 'A')
            throw new Error(
                'Access to this path is granted to Super Admin only<br><a href="/">Home</a>',
            );
