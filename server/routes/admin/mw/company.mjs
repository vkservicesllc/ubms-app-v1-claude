...
const display = (data, ein) => {
    ...
    if (data.category === 'crr') {
        ...
        for (const state in permits) {
            ...
            if (data.stateTax[state]) display.data.stateTax[state] = data.stateTax[state];
        }
...
const ownershipMenu = async (session, category) => {
    ...
    const data = { 'Holding Companies': {}, Individuals: {} },
        names = [];
    ...
    if (!owners.individuals.length) delete data['Individuals'];
        ...
        owners.individuals.forEach(
            (owner, i) =>
                (data['Individuals'][`p:${owner._id}`] =
                    names[i] + (dublicates.includes(names[i]) ? ` (${owner.age})` : '')),
        );
    }
    ...
    if (category === 'hld') delete data['Holding Companies'];
        ...
        if (!owners.companies.length) delete data['Holding Companies'];
            ...
            owners.companies.map(
                (owner) => (data['Holding Companies'][`c:${owner._id}`] = owner.parent.name),
            );
    }

    if (data['Individuals']) data['Individuals'] = sortObjectByValue(data['Individuals']);
    if (data['Holding Companies'])
        data['Holding Companies'] = sortObjectByValue(data['Holding Companies']);
...
export const companyById = async (req, res) => {
    try {
        ...
        const submitProps = {};
        const submitButton = (id, content, style) =>
            formButton({
                type: 'submit',
                class: `button is-fullwidth ${style}`,
                id,
                content,
                disabled: true,
            });
        ...
        if (_id !== 'new') {
            ...
            {
                ({ _id, category, since, ein, duns, busName, coType, alias, website, locked } =
                    data);
            }
            ...
            contentTitle +=
                ' &nbsp;&nbsp;<a id="delete-company-trigger"><i class="fas fa-trash-can has-text-danger is-size-6"></i></a>';
            ...
            if (_ownerId) {
                        ...
                        switch (category) {
                            ...
                            case 'crr':
...

                                {
                                    ...
                                    button.submit.credentials = submitButton(
                                        'credentials-submit',
                                        content,
                                        style,
                                    );
                ...
                /* Address HBS Form & Submit */
                {
                    ...
                    options = updateFormOptions(options, CompanyForm, values, {
                        ...instr,
                        tabs: 5,
                    });
                    if (mailAddrZip) {
                    ...
                    if (mailAddrZip)
                        Object.keys(values)
                            ...
                            .forEach((prop) => {
                                const input =
                                    options[prop]?.text?.input || options[prop].select.input;
            ...
            /* Ownership & Owner HBS Form & Submit */
            {
                ...
                button.add.owner = formButton({
                    class: 'button py-3 is-link',
                    id: 'add-owner-trigger',
...
export const companyByCategoryAndRoute = async (req, res) => {
    try {
        ...
        if (req.params.category !== Company.list.category[category].path[1]) return respond404(res);
        ...
        if (company.lastLogo) {
            ...
            files.forEach((filename, i) => {
                ...
                const caption =
                    fileDate !== '0000-00-00' ? moment(fileDate).format('ll') : 'Launch Date';

                logoList += `\n${t}<figure class="image">`;
                logoList += `\n${t}\t<figcaption><small>${label}:</small> ${caption}`;
                if (files.length - 1 !== i)
                    logoList += `&nbsp;&nbsp;<button class="delete-company-logo" data-filename="${filename}"><i class="has-text-danger-dark fa fa-close"></i></button>`;
                logoList += '</figcaption>';
        ...
        if (company.until)
            hbs.display.status = `<span class="has-text-danger">Permanently Closed <small>(Effective ${moment(company.until).format('ll')})</small></span>`;
        hbs.display.statusTrigger = company.active ? 'Deactivate' : 'Activate';
        hbs.display.statusMessage = company.active
            ? 'place the company on hold'
            : 'release the company from hold';
        hbs.url = {
...
export const companyManagement = async (req, res) => {
    try {
        ...
        const na = '<em class="has-text-danger has-text-weight-normal">N/A</em>';
        const linkAdd = 'fa-plus has-text-link-70',
            linkEdit = 'fa-pen-to-square has-text-success-45';

        company.moment = {
            since:
                company.since !== '0000-00-00'
