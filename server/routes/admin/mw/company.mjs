...
const display = (data, ein) => {
  ...
  for (const group in Company.list.type)
    for (const type in Company.list.type[group]) typeList[type] = Company.list.type[group][type];
          ...
          if (phone) {
            ...
            switch (category) {
              ...
              case 'crr':
                ...
                const { mc, usdot, scac, irp, ifta, iftaJur, stateTax, efs, fleetOne, transflo } =
                  data;
...

                {
                  ...
                  for (const key in permits) values[`${key}Permit`] = stateTax?.[key];

                  catOptions = updateFormOptions(catOptions, CarrierForm, values, {
                    ...instr,
                    tabs: 5,
                  });
                }
