...
router.get('/users', User.mw.verify, (req, res) => {
  try {
    ...
    const fields = {
      ...
      role: ['roleName', 'roleLocation', 'carrierRoleName', 'carrierRoleLocation'],
    };
  ...
  try {
    ...
    try {
      ...
      const display = {
        ...
        gender:
          sex === null ? '<span class="has-text-danger-70">Not specified</span>' : expansion.gender,
      };
      ...
      const input = {
        ...
        unscoped: UserForm.unscoped.checkbox.input({
          checked: unscoped,
