...
router.use((req, res, next) => {
    ...
    res.hbs.set = function (key, params = {}) {
        ...
        if (user) {
            ...
            for (const prop of props) hbs.user[prop] = user[prop];
    ...
    try {
        ...
        if (user.username) hbs.userRegistered = true;
            ...
            const [rows] = await mysql.execute(
                query.user.registration.select('invitedAt', {
                    match: { formId, userId },
                }),
            );
...
router.get('/pass-reset/:_id', async (req, res) => {
    try {
        ...
        const [result] = await mysql.execute(
            query.user.passReset.select('userId', {
...
router.post(
    '/account',
    User.mw.verify,
    [UserForm.newUsername.validate(), UserForm.email.validate(), UserForm.phone.validate()],
    validationCheck,
    async (req, res) => {
        try {
            res.session.user.update(req.body);
...
router.post(
    '/register',
    [UserForm.newUsername.validate(), UserForm.createPassword.validate()],
    validationCheck,
    User.mw.register,
);

router.post('/pass-reset', [UserForm.createPassword.validate()], validationCheck, User.mw.reset);
