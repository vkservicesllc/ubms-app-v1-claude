...
export default async (carrier, application, addresses, violations, accidents, employers) => {
    ...
    const title = applicant.lastName
        ? `${applicant.fullName()} - Driver Application`
        : 'Driver Application Blank';
    ...
    if (carrier) {
        ...
        /* Outline */
        {
            ...
            coverPage.drawLine({
                start: { x: marginX + coverPadding, y: y - height + marginY * 2 + coverPadding },
                    ...
                    x: width - marginX - coverPadding,
                    y: y - height + marginY * 2 + coverPadding,
                },
                color: color.line,
            });
            coverPage.drawLine({
        ...
        /* Intro */
        {
            ...
            text +=
                'This information is essential for evaluating your qualifications and ensuring compliance with applicable regulations. ';
            ...
            lines.forEach((line, i) => {
                coverPage.drawText(line, {
                    x: marginX + gap * 2 + (!i ? gap * 2 : 0),
                    y,
                    font: font.label,
                    size: size.label * 1.55,
                    color: color.section,
                });
                y -= gap * 2;
            });
        ...
        /* Applicant */
        {
            ...
            const age = application?.dob
                ? calculateYearAge(
                      application.dob,
                      moment(application.submittedAt).format('YYYY-MM-DD'),
                  ) + ''
                : '';
            text = 'Age:';
            textWidth = font.label.widthOfTextAtSize(text, size.label * 1.4);
            ...
            coverPage.drawText(
                application.finishedAt ? moment(application.finishedAt).format(dateFormat) : '',
                {
                    x: x + textWidth + gap + 2,
                    y: y + 2,
                    font: font.value,
    ...
    /* Section 2 */
    {
        ...
        textWidth = font.label.widthOfTextAtSize(text, size.label);
        drawCheckBox(page1, marginX + padding, y, !!country, color);
        page1.drawText(text, {
            x: marginX + padding + 15,
            y: y + 1,
            font: font.label,
            size: size.label,
            color: color.label,
        ...
        text = 'Country:';
        page1.drawText(text, {
            x: marginX + padding + 15 + textWidth + gap * 1.4,
            y: y + 1,
            font: font.label,
            size: size.label,
            color: color.label,
        ...
        page1.drawText(country ? Geography.list.country[country] : '', {
            x: marginX + padding + 15 + textWidth + gap * 1.4 + padding * 2,
            y: y + 2,
            font: font.value,
            size: size.value,
            color: color.value,
    ...
    /* Section 3 */
    {
        ...
        /* Row 2 */
        {
            ...
            vLineX += gap;
            text = 'Commercial';
            drawCheckBox(page1, marginX + vLineX, y - fieldHeight / 1.5, commercial, color);
            page1.drawText(text, {
                x: marginX + vLineX + 15,
                y: y - fieldHeight / 1.65,
                font: font.label,
            ...
            page1.drawText('Endorsements', {
                x: marginX + vLineX + gap,
                y: y - offset.labelY - fieldHeight,
                font: font.label,
                size: size.label,
                color: color.label,
            });
        ...
        /* Questions */
        {
            ...
            vLineX = padding;
            page1.drawText(
                'Are you currently taking any medication that may impair your ability to safely operate a commercial motor vehicle?',
                {
                    x: marginX + vLineX,
                    y: y + 1,
                    font: font.label,
    ...
    /* Section 4 */
    {
        ...
        vLineX = padding;
        page1.drawText(
            'Have you ever been arrested in connection with impaired or intoxicated driving (DUI/DWI)?',
            {
                x: marginX + vLineX,
                y: y + 1,
                font: font.label,
        ...
        vLineX = padding;
        page1.drawText(
            'Have you ever been charged with or found guilty of a misdemeanor or felony offense?',
            {
                x: marginX + vLineX,
                y: y + 1,
                font: font.label,
        ...
        vLineX = padding;
        page1.drawText(
            'Within the past 5 years, have you ever failed or refused to take a DOT-required drug or alcohol test',
            {
                x: marginX + vLineX,
                y: y + 1,
                font: font.label,
    ...
    /* Section 5 */
    {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            if (violation) {
                if (violation !== 'other')
                    violationLoop: for (const group in Application.list.violation) {
            ...
            page2.drawText('(Description)', {
                x: marginX + padding + textWidth + 2,
                y: y - offset.labelY,
                font: font.label,
                size: size.label * 0.75,
                color: color.label,
            });
    ...
    /* Section 6 */
    {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            if (collision) {
                if (collision !== 'other')
                    collisionLoop: for (const group in Application.list.collision) {
            ...
            text = 'Injuries';
            drawCheckBox(page2, marginX + vLineX, y - fieldHeight / 1.5, injuries === 'Yes', color);
            page2.drawText(text, {
                x: marginX + vLineX + 15,
                y: y - fieldHeight / 1.65,
                font: font.label,
    ...
    /* Section 7 */
    {
        ...
        y -= fieldHeight / 1.7;
        drawCheckBox(page2, marginX + padding, y, experience?.cmv === true, color);
        ...
        /* Row 2 */
        {
            ...
            for (const vhl in straight) {
                ...
                drawCheckBox(
                    ...
                    marginX + vLineX,
                    y - fieldHeight / 1.5,
                    vehicles.straight.includes(vhl),
                    color,
                );
                page2.drawText(text, {
                    x: marginX + vLineX + 15,
    ...
    /* Section 9 */
    {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            if (!i || i === 5) {
                ...
                textWidth = font.section.widthOfTextAtSize(text, size.section);
                text =
                    '(All jobs from the past 3 years and commercial experience from the past 10 years)';
                emplPages[page].drawText(text, {
                    x: marginX + textWidth + gap,
                    y,
                    font: font.label,
                    size: size.label,
                    color: color.label,
            ...
            let employer, phone, address, startedOn, position, earnings, fmcsr, dotDat, leftOn, rfl;
            if (prevEmployer)
                ({
                    employer,
                    phone,
            ...
            emplPages[page].drawText(
                employer ? (leftOn ? moment(leftOn).format(dateFormat) : 'Still Employed') : '',
                {
                    x: vLineX + padding,
                    y: y - offset.valueY,
                    font: font.value,
    ...
    /* Section 10 */
    {
...

        {
            ...
            textWidth = font.label.widthOfTextAtSize(text, size.label);
            vLineX = padding + textWidth + 15 + gap * 0.8;
    ...
    /* Section 13 */
    {
        const person = application?.beneficiary?.lastName
            ? new Person(application.beneficiary)
            : {};
    ...
    y -= gap / 1.8;
    text =
        'I authorize the employer to verify all statements made in this application, including but not limited to employment history, references, ';
    text +=
        'and personal records such as Motor Vehicle Reports (MVRs), Safety Performance History, and other records required under DOT or FMCSA regulations.';
    lines = wrapText(text, width, font.label, size.label, marginX);
    lines.forEach((line) => {
        page5.drawText(line, {
            x: marginX + padding,
    ...
    y -= gap / 1.8;
    text =
        'I understand and acknowledge that, unless otherwise defined by applicable law, employment with this organization is "at-will", ';
    text +=
        'meaning I may resign at any time, and the company may terminate my employment at any time, with or without cause or notice. ';
    text +=
        'I further understand that no supervisor, recruiter, or manager has authority to alter this employment relationship ';
    ...
    y -= gap / 1.8;
    text =
        'If employed, I agree to comply with all company policies, safety protocols, and DOT/FMCSA regulations. ';
    text +=
        'I understand that any false, misleading, or incomplete information provided during the application or ';
    text +=
        'interview process may result in disqualification or immediate termination of employment.';
    lines = wrapText(text, width, font.label, size.label, marginX);
    lines.forEach((line) => {
        page5.drawText(line, {
            x: marginX + padding,
