...
import {
    ssn as formatSsn,
    tel as formatTel,
} from '../../../../../client/global/modules/tools/utils/formatter.mjs';
...
export default async (employment = {}, method) => {
    ...
    const { carrierCompanyId, carrier, carrierPhone, carrierFax, carrierAddress, carrierLogo } =
        application;
    ...
    /* Consent Page */
    {
        ...
        textWidth = font.title.widthOfTextAtSize(text, size.title * 0.85);
        consentPage.drawText(text, {
            x: width / 2 - textWidth / 2,
            y,
            font: font.title,
            size: size.title * 0.85,
            color: color.title,
        });
        y -= fieldHeight / 1.5;
        ...
        x = marginX;
        y -= size.value * 1.5;
        consentPage.drawText('Printed Name (First, MI, Last)', {
            x: x + 2,
            y,
            font: font.label,
            size: size.label * 0.9,
        ...
        x += 240 + gap;
        consentPage.drawText('Social Security Number', {
            x: x + 2,
            y,
            font: font.label,
            size: size.label * 0.9,
        ...
        x = marginX;
        y -= size.value * 1.5;
        consentPage.drawText('Signature', {
            x: x + 2,
            y,
            font: font.label,
            size: size.label * 0.9,
        ...
        x += 240 + gap;
        consentPage.drawText('Date', { x: x + 2, y, font: font.label, size: size.label * 0.9 });
        consentPage.drawLine({
            start: { x, y: y + 9 },
            end: { x: x + 120, y: y + 9 },
            color: color.line,
        ...
        x += 300 + gap / 1.5;
        consentPage.drawText('(hereinafter referred to as "Carrier").', {
            x,
            y,
            font: font.label,
            size: size.text,
        ...
        textWidth = font.title.widthOfTextAtSize(text, size.title * 0.9);
        consentPage.drawText(text, {
            x: width / 2 - textWidth / 2,
            y,
            font: font.title,
        ...
        text =
            'Pursuant to 49 CFR Part 391.23, I hereby authorize the release of information from my DOT-regulated drug and alcohol testing ';
        text +=
            'records by the carriers (company/school) listed above to Carrier, or to its designated agents, ';
        text +=
            'including HireRight and DriverFacts, for the sole purpose of transmitting such records to Carrier.';
        ...
        const numIdent = 15;
        consentPage.drawText(
            'This authorization includes the release of information concerning the following within the past three (3) years:',
            {
                x,
                y,
                font: font.label,
        ...
        x += numIdent;
        consentPage.drawText('Alcohol test results of 0.04 or higher', {
            x,
            y,
            font: font.label,
            size: size.text,
        ...
        x += numIdent;
        consentPage.drawText('Verified positive drug test results', {
            x,
            y,
            font: font.label,
            size: size.text,
        ...
        x += numIdent;
        consentPage.drawText(
            'Refusals to be tested (including adulterated or substituted results)',
            {
                x,
                y,
                font: font.label,
        ...
        x += numIdent;
        consentPage.drawText('Other violations of DOT drug and alcohol testing regulations', {
            x,
            y,
            font: font.label,
            size: size.text,
        ...
        x += numIdent;
        consentPage.drawText(
            'Information obtained from previous employers regarding drug and alcohol violations',
            { x, y, font: font.label, size: size.text, color: color.section },
        );
        x = marginX + gap * 2;
        y -= fieldHeight * 0.75;
        consentPage.drawText('6.', {
            x,
            y,
            font: font.label,
            size: size.text,
        ...
        x += numIdent;
        consentPage.drawText(
            'Documentation of completion of the return-to-duty process, if applicable',
            { x, y, font: font.label, size: size.text, color: color.section },
        );
        ...
        x += numIdent;
        consentPage.drawText(
            'Dates of negative drug and/or alcohol test results and tests below 0.04',
            { x, y, font: font.label, size: size.text, color: color.section },
        );
        x = marginX + gap * 2;
        y -= fieldHeight * 0.75;
        ...
        x += numIdent;
        consentPage.drawText(
            'The name and contact information of any Substance Abuse Professional who evaluated me within',
            { x, y, font: font.label, size: size.text, color: color.section },
        );
        y -= fieldHeight * 0.75;
        x = marginX + gap * 2 + numIdent;
        consentPage.drawText('the past three (3) years', {
            x,
            y,
            font: font.label,
            size: size.text,
        ...
        x += numIdent;
        consentPage.drawText(
            'Employment-related information including previous employers, dates of employment, reasons for',
            { x, y, font: font.label, size: size.text, color: color.section },
        );
        y -= fieldHeight * 0.75;
        x = marginX + gap * 2 + numIdent;
        consentPage.drawText(
            'termination, work history, accidents, academic history, and professional credentials',
            { x, y, font: font.label, size: size.text, color: color.section },
        );
        ...
        text =
            'I understand that, in connection with my application for employment or continued employment (including contract services) with Carrier, ';
        text +=
            'consumer reports may be requested from HireRight and DriverFacts. These reports may include information such as employment history, driving records, ';
        text +=
            "criminal records, credit history, workers' compensation claims, and other public record information maintained by federal, state, and other agencies.";
        lines = wrapText(text, width, font.label, size.text, marginX, padding);
        lines.forEach((line, i) => {
            consentPage.drawText(line, {
                x,
    ...
    if (carrierLogo && files.length) {
        ...
        verifPage.drawImage(img, {
            x: width - marginX - drawWidth,
            y: y - drawHeight + gap * 1.75,
            width: drawWidth,
            height: drawHeight,
        });
    }
    ...
    verifPage.drawText(formatTel(carrierPhone), {
        x: x + textWidth + gap / 2,
        y,
        font: font.title,
        size: size.title,
    ...
    text = 'Fax:';
    verifPage.drawText(text, { x, y, font: font.title, size: size.subtitle, color: color.title });
    verifPage.drawText(carrierFax ? formatTel(carrierFax) : '', {
        x: x + textWidth + gap / 2,
        y,
        font: font.title,
        size: size.title,
    ...
    y -= fieldHeight;
    verifPage.drawText(
        'Federal Regulations (49 CFR Parts 40, 382, and 391) require prior employers to respond to this inquiry.',
        {
            x,
            y,
            font: font.title,
    ...
    verifPage.drawText(
        'The above-named applicant has submitted an application for a driver position with our company and has indicated that they were',
        {
            x,
            y,
            font: font.label,
    ...
    text =
        "Is the employment information listed above accurate according to your company's records?";
    textWidth = font.label.widthOfTextAtSize(text, size.label);
    verifPage.drawText(text, { x, y, font: font.label, size: size.label });
    x += textWidth + gap * 1.2;
    ...
    x += checkGap;
    text = 'Yes';
    textWidth = font.value.widthOfTextAtSize(text, size.value * 0.9);
    verifPage.drawText(text, { x, y, font: font.value, size: size.value * 0.9 });
    x += textWidth + gap;
    drawRadio(verifPage, x, y - 2, false, color, size.check);
    x += checkGap;
    text = 'No';
    ...
    drawRadio(verifPage, x, y - 2, false, color, size.check);
    x += checkGap;
    verifPage.drawText('Review', { x, y, font: font.value, size: size.value * 0.9 });
    ...
    for (const t in Employment.list.vehicle.type) {
        ...
        x += checkGap;
        textWidth = font.value.widthOfTextAtSize(type, size.value * 0.9);
        verifPage.drawText(type, { x, y, font: font.value, size: size.value * 0.9 });
        x += textWidth + gap;
    }
    text = 'Other';
    textWidth = font.value.widthOfTextAtSize(text, size.value * 0.9);
    ...
    for (const st in Employment.list.vehicle.semiTrailer) {
        ...
        drawCheckBox(verifPage, x, y - 2, false, color, size.check);
        x += checkGap;
        textWidth = font.value.widthOfTextAtSize(trailer, size.value * 0.9);
        verifPage.drawText(trailer, { x, y, font: font.value, size: size.value * 0.9 });
        x += textWidth + gap;
    }
    text = 'Other';
    textWidth = font.value.widthOfTextAtSize(text, size.value * 0.9);
    ...
    verifPage.drawText(text, { x, y, font: font.subsection, size: size.section * 0.9 });

    x = marginX;
    y -= fieldHeight / 1.2;

    text =
        "In the three years prior to the employee's signature date on this release, for DOT-regulated testing:";
    verifPage.drawText(text, { x, y, font: font.label, size: size.label });

    y -= fieldHeight / 1.4;

    const lText =
        'If you answered "Yes" to any of the above, did the employee complete the return-to-duty process?';
    ...
    x += numWidth;
    verifPage.drawText(
        'Did the employee commit any other violations of DOT drug and alcohol testing regulations?',
        {
            x,
            y,
            font: font.label,
    ...
    x += numWidth;
    verifPage.drawText(
        'Did a previous employer report a drug or alcohol rule violation regarding this employee?',
        {
            x,
            y,
            font: font.label,
    ...
    verifPage.drawText('Printed Name', { x: x + 2, y, font: font.label, size: size.label * 0.9 });
    verifPage.drawLine({
        start: { x, y: y + 9 },
        end: { x: x + 240, y: y + 9 },
        color: color.line,
    ...
    x += 240 + gap;
    verifPage.drawText('Signature', { x: x + 2, y, font: font.label, size: size.label * 0.9 });
    verifPage.drawLine({
        start: { x, y: y + 9 },
        end: { x: x + 200, y: y + 9 },
        color: color.line,
    ...
    x += 200 + gap;
    verifPage.drawText('Date', { x: x + 2, y, font: font.label, size: size.label * 0.9 });
    verifPage.drawLine({
        start: { x, y: y + 9 },
    ...
    verifPage.drawText('Title, Phone, Email', {
        x: x + 2,
        y,
        font: font.label,
        size: size.label * 0.9,
    ...
    x += 240 + gap + 200 + gap;
    verifPage.drawText('USDOT', { x: x + 2, y, font: font.label, size: size.label * 0.9 });
    verifPage.drawLine({
        start: { x, y: y + 9 },
        end: { x: width - marginX, y: y + 9 },
        color: color.line,
    ...
    text = 'Comments:';
    textWidth = font.label.widthOfTextAtSize(text, size.label * 0.9);
    verifPage.drawText(text, { x: x + 2, y, font: font.label, size: size.label * 0.9 });
    verifPage.drawLine({
