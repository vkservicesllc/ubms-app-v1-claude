...
router.get('/driver/application/blank/:route?', fileLoggedOut, Team.mw.verify, async (req, res) => {
  try {
    ...
    if (!withPrivileges('d:drv/apl', 'create', permissions, DS)) return respond404(res);
...
router.get('/driver/application/:formId', fileLoggedOut, Team.mw.verify, async (req, res) => {
  ...
  try {
    ...
    const permissions = await user.permissions(res.session);
    if (!withPrivileges('f:drv/apl', 'download', permissions, DS)) return res.redirect(aplUrl);
...
router.get(
  '/driver/application/:formId/consent/psp',
  fileLoggedOut,
  Team.mw.verify,
  async (req, res) => {
    const { formId } = req.params;
    ...
    try {
      ...
      if (
        ...
        (team && application._teamId !== team._id)
      )
        return res.redirect(aplUrl);
      ...
      let pdfBytes = await fillPdf(
        '/carrier/driver/consent_psp',
        {
...
router.get(
  '/driver/application/:formId/:target',
  fileLoggedOut,
  Team.mw.verify,
  async (req, res) => {
    ...
    try {
      ...
      const permissions = await user.permissions(res.session);
      if (!withPrivileges('f:drv/sup', 'download', permissions, DS)) return res.redirect(aplUrl);
...
router.get(
  '/driver/application/previous-employment/verification/:method?',
  fileLoggedOut,
  Team.mw.verify,
  async (req, res) => {
    try {
      ...
      const permissions = await user.permissions(res.session);
      if (!withPrivileges('f:drv/emp', 'download', permissions, DS)) return res.redirect(aplUrl);
