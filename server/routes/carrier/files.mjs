...
export const fileLoggedOut = async (req, res, next) => {
  ...
  if (!user)
    return res.send(
      'Your session has expired, so you can no longer view this file.<br/>Please log in using another tab and refresh this page to regain access.',
    );
...
router.post(
  '/delete/driver/application/:formId',
  fileLoggedOut,
  Team.mw.verify,
  async (req, res) => {
    try {
      ...
      if (!withPrivileges('f:drv/sup', 'delete', permissions, DS)) return res.redirect(aplUrl);
...
router.post(
  '/skip/driver/application/:formId/:target',
  fileLoggedOut,
  Team.mw.verify,
  async (req, res) => {
    try {
      ...
      const permissions = await user.permissions(res.session);
      if (!withPrivileges('f:drv/sup', 'upload', permissions, DS)) return res.redirect(aplUrl);
