...
router.get('/applications', User.verify, Team.verify, async (req, res) => {
    ...
    if (hbs.permissions.create) {
        ...
        const { class: aplClass, emailId, addr1Id, addr2Id, zipId, cityId, stateId } = formSelectors.driver
        ...
        hbs.input = {
            // email: ContactInput.email({ id: emailId, placeholder: "Applicant's Email", required: true }),
