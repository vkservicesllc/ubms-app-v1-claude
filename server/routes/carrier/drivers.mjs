...
import {
  ...
  inPEnvironment,
  withPrivileges,
} from '../../tools/core/user/permissions.mjs';
...
router.get('/applications', User.mw.verify, Team.mw.verify, async (req, res) => {
  try {
    ...
    const { DS } = user;
    const permissions = await user.permissions();
    if (!inPEnvironment('d:drv/apl', permissions, DS)) return res.redirect(res.session.defUrl);
    ...
    hbs.permissions = {};
    privs.forEach(
      (priv) => (hbs.permissions[priv] = withPrivileges('d:drv/apl', priv, permissions, DS)),
    );
    ...
    if (hbs.permissions.create) {
      ...
      //? if (!DS)
      if (true) hbs.applicationUrl += `&rec=${user._simpleId}`;
...
router.get('/application/:formId/e-form', User.mw.verify, Team.mw.verify, async (req, res) => {
  try {
    ...
    if (!withPrivileges('d:drv/apl', 'modify', permissions, DS)) return res.redirect(aplUrl);

    const { formId } = req.params;
    const application = await Application.fetch(res.session, { formId }, { hideSensitive: false });

    if (!application || application.condition === 'h' || application.rehire)
    ...
    if (_carrierId) {
      hbs.carrier =
        '<span class="ui red text"><i class="ui ban icon"></i> Failed to fetch carrier</span>';
    ...
    if (_userId) {
      hbs.recruiter =
        '<span class="ui red text"><i class="ui ban icon"></i> Failed to fetch recruiter</span>';
    ...
    hbs.downloadLinks = {
      //! index 3 is download name
      dl: [
        `${filePath}/drivers-license`,
        "Driver's License",
        `[${formId}] Driver's License (${fullName})`,
      ],
      mec: [
        `${filePath}/medical-certificate`,
        'Medical Certificate',
        `[${formId}] Medical Certificate (${fullName})`,
      ],
      leg: [
        `${filePath}/legal-document`,
        'Legal Documents',
        `[${formId}] Legal Documents (${fullName})`,
      ],
      ssc: [
        `${filePath}/social-security-card`,
        'Social Security Card',
        `[${formId}] Social Security Card (${fullName})`,
      ],
    ...
    hbs.checklist = {
      ...
      ssc: {
        ...
        status: uploaded.ssc ? status.verified : skipped.ssc ? status.skipped : status.pending,
      },
      leg: {
        check: uploaded.leg
          ? checkStyle.doubleChecked
          : skipped.leg
            ? checkStyle.skipped
            : checkStyle.unchecked,
        status: uploaded.leg ? status.verified : skipped.leg ? status.skipped : status.pending,
      },
      reg: {
    ...
    if (passed.dl || passed.mec || passed.leg || passed.ssc) {
      ...
      if (passed.dl && passed.mec && passed.leg && passed.ssc) hbs.step.documents = ' completed ';
    }
    ...
    if (vhlProp) hbs.position += ` <small>(${Application.list.vhlType[cdlRole][vhlProp]})</small>`;
    ...
    /* WORKFLOW */
    {
      ...
      if (application._userId) options.user = { hidden: { input: { value: application._userId } } };
      ...
      if (application._teamId) options.team = { hidden: { input: { value: application._teamId } } };
    ...
    /* FILES */
    {
      ...
      if (hbs.fileTab) {
        ...
        for (const prop in carrierPrivs['f:drv'].groups) {
          ...
          if (privs === '*')
            privileges.file.forEach(
              (priv) =>
                (hbs.filePerms[prop][priv] = withPrivileges(
                  `f:drv/${prop}`,
                  priv,
                  permissions,
                  DS,
        ...
        hbs.uploads = {};
        for (const prop in application.uploads) hbs.uploads[prop] = application.uploads[prop][0];
    ...
    /* POSITION */
    {
      ...
      if (!cdlRole) {
        ...
        for (const len in lenData) {
          const option = lenData[len]
            .replace('(', '<small><span class="ui text grey">(')
            .replace(')', ')</span></small>');
    ...
    /* DRIVER's LICENSE */
    {
      ...
      options.dlDeniedExpl = {
        text: { input: { rows: 2, placeholder: ' ' }, label: { content: 'Details' } },
      };
      options.dlRevokedExpl = {
        text: { input: { rows: 2, placeholder: ' ' }, label: { content: 'Details' } },
      };
    }
    ...
    /* MEDICAL CARD */
    {
      ...
      options.noMec = {
        checkbox: {
          label: {
            content:
              '<span class="ui dark orange text"><i class="exclamation triangle icon"></i> Unavailable at the time of submission</span>',
          },
        },
      };
    }
    ...
    /* EXPERIENCE */
    {
      ...
      for (let i = 0; i < 7; i++) {
        const content =
          `${appliedOn.clone().subtract(--j, 'days').format('dddd/MMM D, YYYY')}`.replace(
            '/',
            '<br/>',
          );
    ...
    /* PREVIOUS EMPLOYERS */
    {
      hbs.length.prevEmployment = (
        await Employment.fetch(res.session, { appId: application.id })
      ).length;
    ...
    /* BUSINESS */
    {
      ...
      options.inactiveLLC = {
        checkbox: {
          label: {
            content:
              '<span class="ui dark orange text"><i class="exclamation triangle icon"></i> No currently active LLC</span>',
          },
        },
      };
...
router.get(
  '/application/:formId/e-form/:target',
  User.mw.verify,
  Team.mw.verify,
  async (req, res) => {
    try {
      ...
      switch (target) {
        case 'prior-residence':
          ...
          hbs.addrMaxDate = moment(application.address.since)
            .clone()
            .subtract(1, 'day')
            .format('YYYY-MM-DD');
        ...
        case 'prev-employers':
          ...
          fields.forEach((field) => {
            let prop = 'text';
            if (Array.isArray(field)) [field, prop] = field;

            emplOptions[field] = { [prop]: { input: { disabled: false } } };
          });
          ((emplOptions.RFL.text.input.rows = 2), (emplOptions.RFL.text.input.placeholder = ' '));
...
router.get('/previous-employments', User.mw.verify, Team.mw.verify, async (req, res) => {
  try {
    ...
    const { DS } = user;
    const permissions = await user.permissions(res.session);
    if (!inPEnvironment('d:drv/emp', permissions, DS)) return res.redirect(res.session.defUrl);
    ...
    hbs.permissions = {};
    privs.forEach(
      (priv) => (hbs.permissions[priv] = withPrivileges('d:drv/emp', priv, permissions, DS)),
    );
    ...
    if (hbs.permissions.create || hbs.permissions.modify || hbs.permissions.update) {
      ...
      if (hbs.permissions.create || hbs.permissions.modify) {
        ...
        ((options.RFL.text.input.rows = 3), (options.RFL.text.input.placeholder = ' '));
      } else {
        // make html info only
      }
      ...
      if (hbs.permissions.update) {
        ...
        const methods = Employment.list.inquiryMethod,
          responses = Employment.list.inquiryResponse;
        ...
        const termTypes = Employment.list.termType,
          rehireTypes = Employment.list.rehire;
...
router.get('/hired', User.mw.verify, Team.mw.verify, async (req, res) => {
  try {
    ...
    const { DS } = user;
    const permissions = await user.permissions(res.session);
    if (!inPEnvironment('d:drv/drv', permissions, DS)) return res.redirect(res.session.defUrl);
