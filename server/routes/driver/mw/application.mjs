...
export const applicationStart = async (req, res, next) => {
    try {
        ...
        if ((formId && !application) || application?._driverId)
...
export const applicationProgress = async (req, res, next) => {
    try {
...

        {
            ...
            options = updateFormOptions(options, ApplicationForm, values, {
                ...formInstr,
                tabs: 12,
            });
            options.addrState.select.input.options = { valOpt: true };
        ...
        if (step >= 0) {
            ...
            for (const prop of ['yes', 'no']) {
                options.livedAbroad.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
                options._livedAbroad.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
            }
            ...
            if (application.address.enough === false) {
                ...
                options.livedAbroad.radio.yes.input.checked =
                    application.address.livedAbroad === true;
                options.livedAbroad.radio.no.input.checked =
                    application.address.livedAbroad === false;

                values.country = application.address.country;

                if (application.address.livedAbroad === null)
                    hbs.scrollPoint.priorAddr = scrollAttr;
            }

            updateFormOptions(options, ApplicationForm, values, { ...formInstr, tabs: 13 });
            options._addrState.select.input.options = { valOpt: true };
            options._addrEnough = {
                hidden: { input: { class: 'driver-application-prev-addr-enough-hidden-input' } },
            };
            options._addrCurrentSince = {
                hidden: { input: { class: 'driver-application-prev-addr-since-hidden-input' } },
            };
        ...
        if (step >= 1) {
            ...
            const values = {
                ...
                dlIss: application?.dl?.issuedOn
                    ? moment(application.dl.issuedOn).format('MM/DD/YYYY')
                    : null,
                dlExp: application?.dl?.expiresOn
                    ? moment(application.dl.expiresOn).format('MM/DD/YYYY')
                    : null,
            ...
            Object.keys(placeholders).forEach(
                (prop) => (options[prop].text.input.placeholder = placeholders[prop]),
            );
            ...
            for (const prop of ['yes', 'no']) {
                options.dlCommercial.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
                options.dlDenied.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
                options.dlRevoked.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
            }
            ...
            if (options.dlCommercial.radio.yes.input.checked)
        ...
        if (step >= 2) {
            ...
            const values = {
                mecExp: application?.mec?.expiresOn
                    ? moment(application.mec.expiresOn).format('MM/DD/YYYY')
                    : null,
                mecIss: application?.mec?.issuedOn
                    ? moment(application.mec.issuedOn).format('MM/DD/YYYY')
                    : null,
            ...
            options.noMec = {
                checkbox: { input: { ...checkProps.input }, label: { ...checkProps.label } },
            };
            options.underMeds = { radio: {} };
            for (const prop of ['yes', 'no'])
                options.underMeds.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
            ...
            if (hbs.medCard && step > 2 && !application._mecId) {
                ...
                fields.forEach((prop) => {
                    if (options[prop].text) options[prop].text.input.disabled = false;
                });
            if (values.medList) {
        ...
        if (step >= 3) {
            ...
            for (const prop of ['yes', 'no']) {
                options.dui.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
                options.duiInDecade.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
                options.criminal.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
                options.dotDat.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
                options.citations.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
            }
            ...
            if (application.citations === true) hbs.citationsDisplay = '';
        ...
        if (step >= 4) {
            ...
            for (const prop of ['yes', 'no']) {
                options.accidents.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
                options._accInjuries.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
                options._accFatalities.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
            }
            ...
            if (application.accidents === true) hbs.accidentsDisplay = '';
        ...
        if (step >= 5) {
            ...
            for (const prop of ['yes', 'no']) {
                options.cmvExp.radio[prop] = {
                    input: { ...checkProps.input, disabled },
                    label: { ...checkProps.label },
                };
                options.cdlSchool.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
            }
            ...
            for (const prop in Application.list.vehicle.straight) {
                ...
                options.straightExp.checkbox[prop] = {
                    input: { ...checkProps.input, checked, disabled },
                    label: { ...checkProps.label },
                };
            }
            ...
            for (const prop in Application.list.vehicle.semi) {
                const checked =
                    application?.experience?.cmv === true &&
                    application?.experience?.vehicles?.semi?.includes(prop);
                options.semiExp.checkbox[prop] = {
                    input: { ...checkProps.input, checked, disabled },
                    label: { ...checkProps.label },
                };
            }
            ...
            options.vanExp = {
                checkbox: {
                    input: {
                        ...checkProps.input,
                        disabled,
                        checked: application?.experience?.vehicles?.misc?.includes('van'),
                    },
                    label: { ...checkProps.label },
                },
            };
            options.hotshotExp = {
                checkbox: {
                    input: {
                        ...checkProps.input,
                        disabled,
                        checked: application?.experience?.vehicles?.misc?.includes('hotshot'),
                    },
                    label: { ...checkProps.label },
                },
            };
            options.tandemExp = {
                checkbox: {
                    input: {
                        ...
                        checked:
                            application?.experience?.cmv === true &&
                            application?.experience?.vehicles?.misc?.includes('tandem'),
                    },
                    label: { ...checkProps.label },
                },
            };
            ...
            if (application?.experience?.cmv === false)
                hbs.cmvExpDisplay = ' style="display: none;"';
        ...
        if (step >= 6) {
            ...
            for (const prop of ['yes', 'no']) {
                options.prevEmployed.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
                emplOptions.FMCSR.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
                emplOptions.dotDat.radio[prop] = {
                    input: { ...checkProps.input },
                    label: { ...checkProps.label },
                };
            }
            ...
            emplOptions = updateFormOptions(emplOptions, EmploymentForm, fields, {
                ...formInstr,
                tabs: 7,
            });
        ...
        if (step >= 7) {
            ...
            options.operType = {
                radio: {
                    solo: {
                        input: {
                            ...checkProps.input,
                            checked: application?.preference?.operType === 's',
                        },
                        label: { ...checkProps.label },
                    },
                    team: {
                        input: {
                            ...checkProps.input,
                            checked: application?.preference?.operType === 't',
                        },
                        label: { ...checkProps.label },
                    },
                },
            ...
            for (const prop in Application.list.haulRegion) {
                ...
                options.haulRegion.checkbox[prop] = {
                    input: { ...checkProps.input, checked },
                    label: { ...checkProps.label },
                };
            }
            ...
            for (const prop in Application.list.vehicle.semi) {
                ...
                options.equipmentType.checkbox[prop] = {
                    input: { ...checkProps.input, checked },
                    label: { ...checkProps.label },
                };
            }
            ...
            const values = {
                ...
                teamPhone: application?.preference?.teamPhone
                    ? formatTel(application?.preference?.teamPhone)
                    : null,
        ...
        if (step >= 8) {
            ...
            if (application.position === 'OO') {
                ...
                if (!cdlRole) {
                    ...
                    hbs.currentVhlLenDisplay = values.currentVhlLen
                        ? ''
                        : ' style="display: none;"';
                ...
                const wTrailer = applicationSettings.vhlType_wTrailer.includes(
                ...
                if (wTrailer) {
                    ...
                    options.currentVhlTrailer.radio[
                        application?.vehicle?.trailer ? 'yes' : 'no'
                    ].input.checked = true;
                }
                ...
                if (!cdlRole) {
                    ...
                    options.currentVhlLen.select.input.data =
                        Application.list.vhlLength.straightBox;
                }
            }
...
export const applicationSummary = async (req, res) => {
    try {
        ...
        if (application.emergency.relation)
            hbs.application.emergency.relation = ` <small>(${application.emergency.relation})</small>`;
        ...
        if (application.mecId) {
            ...
            hbs.application.mec.issuedOn = application.mec.issuedOn
                ? moment(application.mec.issuedOn).format('ll')
                : na('N/A');
        ...
        if (application.experience !== false) {
            ...
            if (application.experience.vehicles) {
                ...
                for (let group in vehicles) {
                    ...
                    if (html) html += '<br/>';
                    if (group !== 'misc')
                        html += `<small class="text-secondary"><b>${groups[group]}:</b></small><br/>`;
            ...
            if (application.experience.mileage)
                hbs.application.experience.mileage =
                    application.experience.mileage.toLocaleString();
            if (application.experience.hours) {
                let total = 0;
                application.experience.hours.forEach((hours) => (total += +hours));
                hbs.application.experience.hourList =
                    application.experience.hours.join(' + ') + ' = ' + total;
            }
        }
        ...
        if (application.cdlSchool !== null) {
            if (application.cdlSchool !== false) {
                ...
                hbs.application.cdlSchool.duration =
                    Application.list.schoolDuration[application.cdlSchool.duration];
                hbs.application.cdlSchool.endDate = moment(application.cdlSchool.endDate).format(
                    'll',
                );
        ...
        hbs.application.preference.teamPhone = application.preference.teamPhone
            ? formatTel(application.preference.teamPhone)
            : na();
        if (application.preference?.haulRegion) {
            const haulRegionList = [];
            application.preference.haulRegion.forEach((region) =>
                haulRegionList.push(Application.list.haulRegion[region]),
            );
            hbs.application.preference.haulRegionList = haulRegionList.join(', ');
        }
        if (application.preference?.haulRegion) {
            const equipmentList = [];
            application.preference.equipmentType.forEach((type) =>
                equipmentList.push(Application.list.vehicle.semi[type]),
            );
            hbs.application.preference.equipmentList = equipmentList.join(', ');
        }
        hbs.application.preference.startPref =
            Application.list.startPref[application.preference.startPref];
        ...
        hbs.application.beneficiary.relationship =
            application.beneficiary.otherRel || application.beneficiary.relation;
        ...
        hbs.application.beneficiary.ssn = application.beneficiary.ssn
            ? formatSsn(application.beneficiary.ssn)
            : na('N/A');
        ...
        if (application.vehicle) {
            ...
            hbs.application.vehicle.type =
                Application.list.vhlType[application.cdlRole][application.vehicle.type];
        }
