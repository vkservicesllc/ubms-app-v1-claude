...
class Company {
        ...
        if (single && !hideRawId) {
            ...
            this.delete = async (target, matchOrIds) => {
                ...
                if (!target) {
                    ...
                    const [ result ] = await mysql.execute(query.company.main.delete({ id }))
            ...
            this.confirm = async () => {
                ...
                const [ result ] = await mysql.execute(query.company.main.update(data, { id: this.id || Company.matchIdHash(this._id) }))
            ...
            this.log = (field, target, since) => classInstance.log(this, new.target, field, { target, since })
    ...
    static create = async ({ user: sessionUser = {}, branch, siteId = null }, body = {}) => {
        ...
        let [ result ] = await mysql.execute(query.company.main.insert(data.main))
        const id = result.insertId

        if (!id) throw new Error('DB Error: Failed to create company')

        { data.name.companyId = id }
        [ result ] = await mysql.execute(query.company.name.insert(data.name))
...
class Owner extends Individual {
    constructor(data = {}, { single = true, session, hideRawId = false, hideSensitive = true }) {
        ...
        if (single && !hideRawId) {
            ...
            this.delete = async () => {
                ...
                const [ result ] = await mysql.execute(query.owner.main.delete({ id }))
                if (!result.affectedRows) return false
    ...
    static create = async ({ user: sessionUser = {} }, body = {}) => {
        ...
        const [ result ] = await mysql(query.owner.main.insert({ personId, createdBy }))
        const id = result.insertId
    ...
    static find = async (session, params = {}) => {
        ...
        if (result?.personId && scope === 'local') {
            ...
            const data = (await mysql.execute(query.owner.main.select('id', {
                match: { personId },
            })))
