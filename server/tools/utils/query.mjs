...
class Query {
    ...
    static unMd5 = (db, table, field, md5Str) =>
        `SELECT ${field} FROM ${db}.${table} WHERE MD5(${field}) = '${md5Str}'`;
    ...
    static select(primaryDb, batch, limit) {
        if (!Array.isArray(batch)) {
            ...
            return new Query(primaryDb, table).select(fields, {
                match,
                search,
                sort,
                group,
        ...
        const { columns, primaryTable, joins, matches, searches, grouper, sorts } = Query.#unbatch(
            primaryDb,
            batch,
        );
    ...
    static hashField(field = {}, table) {
        if ('md5' in field || 'sha1' in field || 'sha2' in field) return Query.#field(field, table);
    }
    ...
    static #field(field, table) {
        ...
        if (Array.isArray(field)) [field, resField] = field;
        ...
        if (typeof field === 'object') {
            if ('aes' in field) {
                ...
                let length = 512,
                    secondLength;
                field = field.sha2;
                if (Array.isArray(field)) [field, length, secondLength] = field;
                if (!resField) resField = field;
                if (resField[0] !== '_') resField = `_${resField}`;
                ...
                if (Array.isArray(field)) [field, resField] = field;
                else resField = prop + capitalizeFirst(field);
                ...
                } else field = `COUNT(DISTINCT ${field})`;
                ...
                field = field.map((item) => {
                    if (item[0] === '^') item = `'${item.replace(/\^/g, '')}'`;
                    else item = Query.#_field(item, table);
                ...
                if (Array.isArray(field[0])) [field, resField] = field;
        ...
        } else field = Query.#_field(field, table);
    ...
    static #value(value) {
        ...
        if (value instanceof Date) value = formatDateToString(value);
            ...
            if ('aes' in value) [value, secret] = value.aes;
    ...
    static #match(match = {}, table) {
        ...
        for (let field in match) {
            ...
            if (field === 'route' || field === 'concat') {
                ...
                field = { [prop]: [fields] };
            } else if (Array.isArray(value)) [value, operator, extension] = combine(value);
            else if (value !== null && typeof value === 'object') {
                if ('aes' in value || 'ip' in value) {
                    ...
                    if (Array.isArray(value)) [value, operator, extension] = combine(value, false);
                    else {
                        value = Query.#value(value);
                        operator = ' != ';
                    }
                    ...
                    if (length) {
                        let secondLength;
                        if (Array.isArray(length)) [length, secondLength] = length;
            ...
            if (value === 'NULL' || value === 'NOT NULL') operator = ' IS ';
    ...
    static #sort(sort, table) {
        if (typeof sort === 'object') {
            if (Array.isArray(sort)) sort = sort.map((item) => resolve(item, table)).join(', ');
            else sort = resolve(sort, table);
    ...
    static #unbatch(primaryDb, batch) {
        ...
        for (const cluster of batch) {
            ...
            if (!db) db = primaryDb;
            if (Array.isArray(table)) [table, asTable] = table;
            else asTable = table;
            if (!fields) fields = [];
            else if (!Array.isArray(fields)) fields = [fields];

            fields = fields.map((field) => (field = Query.#field(field, asTable)));
            ...
            if (primaryTable && join) {
                ...
                if (typeof param3 === 'string') foreignTable = param3;
                else if (typeof param3 === 'number') foreignTable = tables[param3];
                ...
                if (['left', 'right', 'inner', 'outter', null].includes(type)) joiner.type = type;

                joiner.table = Query.#_table(table, db);
                if (asTable !== table) joiner.table += ` AS ${asTable}`;
                joiner.links[0] = `${asTable}.${id} = ${foreignTable}.${foreignId}`;
                    ...
                    if (Array.isArray(purpose)) {
                        ...
                        if (typeof purpose[1] === 'object')
                            for (const field in purpose[1]) {
                                ...
                                if (value)
                                    match += ` AND ${subTable}.${field} = ${Query.#_value(value)}`;
                            }
                    }
                ...
                //! NOT TESTED
                    ...
                    if (Array.isArray(asOfMax)) {
                        ...
                        maxField = 'COALESCE(';
                        if (Array.isArray(asOfMax[1]))
                            asOfMax[1].map((field) => (maxField += `${asOfTable}.${field}, `));
                    ...
                    joiner.table = `(SELECT * FROM ${table}\nWHERE ${Query.#match(foreignMatch)}) AS ${asTable}`;
                ...
                if (Array.isArray(join[3])) {
                    ...
                    join2.map((pieces, i) => {
                        ...
                        joiner.links[i + 1] =
                            `${asTable}.${field} = ${foreignTable}.${foreignField}`;
                    });
                }
            }

            if (!primaryTable) primaryTable = Query.#_table(table, db);

            match = Query.#match(match, asTable);

            if (sort) sort = Query.#sort(sort, asTable); //! UNTESTED
            ...
            if (Array.isArray(search)) {
                ...
                if (searchStr)
                    searches = searches.concat(Query.#search(searchStr, searchFields, asTable)); //! UNTESTED
            }
            if (sort) sorts.push(sort); //! UNTESTED
            if (!grouper && group) grouper = Query.#field(group, asTable);
        }
