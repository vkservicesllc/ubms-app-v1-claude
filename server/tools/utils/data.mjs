...
export async function processData(
  data = {},
  { query, target = 'main', skipLog = false, match = {}, modifiedBy, branch, siteId, debug } = {},
) {
  ...
  if (update) {
    ...
    if (debug?.processData?.fetchQuery)
      console.log(
        '[debug] processData fetchQuery',
        `
${fetchQuery}
`,
      );
    if (debug?.processData?.match) console.log('[debug] processData match', match);
    if (debug?.processData?.currentData)
      console.log('[debug] processData currentData', currentData);
  ...
  for (const field in data) {
    ...
    if (update) {
      ...
      if (updateLog) {
        ...
        updateLog[0].oldData[field] =
          currentValue && encData ? encrypt(currentValue) : processValue(currentValue);
      }
  ...
  if (updateLog && Object.keys(updateLog[0].data).length) {
    if (currentUpdateLog) updateLog = updateLog.concat(currentUpdateLog);
