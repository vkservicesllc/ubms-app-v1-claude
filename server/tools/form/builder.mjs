...
const createForm = (input = {}) => {
    const { selector, target, group, mode, data, keys, defaultClass, id, required, disabled } =
        input;
    ...
    if (
        ...
        form[keyType].input = (props) =>
            formTextInput({ ...input, ...props, ...lockedInput[keyType] });
    ...
    if (validator !== false)
        form.validate = () => {
            ...
            if (typeof validator === 'object') {
                const { caps, sanitizer, rule, match, length, optional, ignoreData, custom } =
                    validator;
