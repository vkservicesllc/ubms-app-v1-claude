...
import {
  ...
  formRadio,
  formCheckbox,
} from '../../../client/global/modules/tools/utils/html/form.mjs';
...
const createForm = (input = {}) => {
  ...
  if (name && (input.multiple === true || (type === 'checkbox' && data))) name += '[]';
  ...
  const initializeInput = (type, key, i) => {
    ...
    if (selector && (target || group)) {
      ...
      else if (typeof defaultClass === 'string') lockedInput[type].defaultClass = [defaultClass];

      //! ATTN
      //? Think about global class if input has customizable value
      if (valueTypes.includes(type)) lockedInput[type].defaultClass.push(selector.class.global);
      ...
      if (valueTypes.includes(type) || (type === 'checkbox' && !input.multiple))
        if (selector.id) lockedInput[type].id = selector.id[type][target];
      if (key) {
    ...
    if (!key) {
      if (label) {
        ...
        if (required && requiredLabel) {
          ...
          if (typeof defaultClass === 'string') lockedLabel[type].defaultClass = [defaultClass];
          else if (!Array.isArray(defaultClass)) lockedLabel[type].defaultClass = [];
      ...
      if (lockedInput[type]?.[key]?.id) lockedLabel[type][key].for = lockedInput[type][key].id;
    }
  };
  ...
  if (
    ['hidden', 'text', 'textarea', 'password', 'email', 'url', 'number'].includes(type) ||
    selectType
  ) {
  ...
  if (selectType) {
    ...
    form.select.input = (props) => {
      ...
      return formSelect(
        { ...input, ...props, ...lockedInput.select },
        props?.data || data || {},
        options,
      );
    };
  ...
  if (type.includes('radio') && data) {
    ...
    propKeys.forEach((key, i) => {
      ...
      form.radio[key].input = (props) =>
        formRadio({ ...input, ...props, ...lockedInput.radio[key] });
  ...
  if (type.includes('checkbox')) {
    ...
    if (data) {
      ...
      propKeys.forEach((key, i) => {
        ...
        form.checkbox[key].input = (props) =>
          formCheckbox({ ...input, ...props, ...lockedInput.checkbox[key] });
        form.checkbox[key].label = () => formLabel(lockedLabel.checkbox[key]);
      });
    } else {
      initializeInput('checkbox');

      form.checkbox.input = (props) =>
        formCheckbox({ ...input, ...props, ...lockedInput.checkbox });
    }
  ...
  if (validator !== false)
    form.validate = () => {
      ...
      if (typeof validator === 'object') {
        ...
        if (required && !disabled && !optional)
          //! Useless in case the default state is disabled (will use it for now)
          chain = chain.notEmpty().withMessage(`"${name}" field can not be empty`);
        else chain = chain.customSanitizer((value) => value || null).optional({ nullable: true });

        if (data && ignoreData !== true) {
          const values = Object.keys(data);

          if (values.length)
            chain = chain.isIn(values).withMessage(`Incorrect value provided in "${name}"`);
        }

        if (caps === true) chain = customSanitizer((value) => value.toUpperCase());

        if (sanitizer) {
          const sanitizers = Array.isArray(sanitizer) ? sanitizer : [sanitizer];

          for (const fn of sanitizers)
            if (typeof fn === 'function') chain = chain.customSanitizer(fn);
        }

        switch (rule) {
          case 'numeric':
            chain = chain.isNumeric().withMessage(`"${name}" field must be numeric`);
            break;

          case 'alphanumeric':
            chain = chain.isAlphanumeric().withMessage(`"${name}" field must be alphanumeric`);
            break;
          ...
          case 'date':
            chain = chain
              .customSanitizer((date) => {
                ...
                return moment(date, ['YYYY-MM-DD', 'MM/DD/YYYY', 'MMM D, YYYY'], true).format(
                  'YYYY-MM-DD',
                );
              })
              .isDate()
        ...
        if (typeof custom === 'function') chain = chain.custom(custom);
...
export const constructForm = (Src, target, options) => {
  ...
  if (checkbox) {
    ...
    const keys = Object.keys(checkbox).filter(
      (key) => key !== 'properties' && key !== 'label' && key !== 'input',
    );
...
export const updateFormOptions = (options, Form, data, instr = {}) => {
  ...
  const { labelClass, labelClassRequired, textClass, textareaClass, selectClass, tabs, disabled } =
    instr;
  ...
  fields.forEach((prop) => {
    ...
    keys.forEach((key) => {
      ...
      } else if (key === 'select') options[prop][key].input = { class: selectClass, tabs };
