...
class User extends Person {
    constructor(
        data = {},
        { single = true, session, hideRawId = false, hideSensitive = true, custom = {} } = {},
    ) {
        ...
        if (single) {
            ...
            this.add = (target, ids = []) => {
                if (!this.session?.user?.id)
                    throw new Error(
                        'User Constructor Method Error [ADD]: Session user not supplied',
                    );
            ...
            this.update = (body) => {
                ...
                if (!session?.user?.id)
                    throw new Error(
                        'User Constructor Method Error [UPDATE]: Session user not supplied',
                    );
                    ...
                    {},
                    {
                        async final(user, body) {
                            user = await User.fetch(
                                ...
                                { _id: user._id },
                                { hideSensitive: false },
                            ); //* Original user does not have a username prop
                            const { status, email } = body;

                            if (email && user.email !== email && !user.username) {
                                if (user.events === undefined)
                                    throw new Error(
                                        'Failed to reinvite user due to missing event property',
                                    );
            ...
            this.delete = (target, ids = []) => {
                if (!this.session?.user?.id)
                    throw new Error(
                        'User Constructor Method Error [DELETE]: Session user not supplied',
                    );

                return classInstance.delete(this, new.target, target, ids, async () => {
                    const config = User.config();

                    const update = await processData(
                        { username: null, email: null, phone: null, condition: 'I' },
                        {
                            query: config.query,
                            match: { id: this.id },
                            skipLog: true,
            ...
            this.reset = async () => {
                if (!this.session?.user?.id)
                    throw new Error(
                        'User Constructor Method Error [RESET]: Session user not supplied',
                    );
                ...
                let [result] = await mysql.execute(
                    query.user.passReset.insert({ resetId, userId, createdBy }),
                );
                ...
                [result] = await mysql.execute(
                    query.user.main.update({ _passKey: null, passReset: true }, { id: userId }),
                );
            ...
            this.invite = async () => {
                if (!this.session?.user?.id)
                    throw new Error(
                        'User Constructor Method Error [INVITE]: Session user not supplied',
                    );

                const [rows] = await mysql.execute(
                    query.user.registration.select('formId', { match: { userId: this.id } }),
                );
            ...
            this.settings = async (action = 'fetch', data = {}) => {
                ...
                let settings =
                    (await mysql.execute(query.user.main.select('settings', { match })))[0][0]
                        .settings || {};
                ...
                if (action === 'update') {
                    if (!this.session.branch)
                        throw new Error('User Settings Error: Session branch missing');
            ...
            this.url = async (lastUrl) => {
                if (
                    lastUrl.slice(0, 5) === '/api/' ||
                    lastUrl.includes('/files/') ||
                    lastUrl.includes('/image/') ||
                    lastUrl.endsWith('.map')
                )
                    return;
                ...
                await mysql.execute(
                    query.session.main.update({ lastUrl }, { userId, siteId, branch, lastLogin }),
                );
                ...
                */
                ...
                if (updateLog)
                    for (let i = 0; i < updateLog.length; i++) {
                        log.updateLog[i].modifiedBy =
                            names[updateLog[i].modifiedBy] || config.site.name;

                        for (const prop in updateLog[i].data) {
                            switch (prop) {
                                case 'status':
                                    log.updateLog[i].data.status =
                                        User.list.status[updateLog[i].data.status];
                                    log.updateLog[i].oldData.status =
                                        User.list.status[updateLog[i].oldData.status];
                                    break;
                                case 'location':
                                    log.updateLog[i].data.location =
                                        User.list.location[updateLog[i].data.location];
                                    log.updateLog[i].oldData.location =
                                        User.list.location[updateLog[i].oldData.location];
                                    break;
                                case 'condition':
                                    log.updateLog[i].data.condition =
                                        User.list.condition[updateLog[i].data.condition];
                                    log.updateLog[i].oldData.condition =
                                        User.list.condition[updateLog[i].oldData.condition];
                                    break;
                                case 'gender':
                                    log.updateLog[i].data.gender =
                                        Individual.list.gender[updateLog[i].data.gender] || null;
                                    log.updateLog[i].oldData.gender =
                                        Individual.list.gender[updateLog[i].oldData.gender] || null;
                                case 'unscoped':
                                    log.updateLog[i].data.unscoped =
                                        !!log.updateLog[i].data.unscoped;
                                    log.updateLog[i].oldData.unscoped =
                                        !!log.updateLog[i].oldData.unscoped;
                            }
    ...
    static #idStr = async (idField, length, queryInst) => {
        ...
        do {
            ...
            const [rows] = await mysql.execute(
                queryInst.select(idField, { match: { [idField]: idStr } }),
            );
    ...
    static #formId = async () =>
        await User.#idStr('formId', inputLength.user.formId.max, query.user.registration);
    static #resetId = async () =>
        await User.#idStr('resetId', inputLength.user.resetId.max, query.user.passReset);

    static create = ({ user: sessionUser = {}, branch, siteId = null }, body, params) => {
        if (!sessionUser.id)
            throw new Error('User Static Method Error [CREATE]: Session user not supplied');
        ...
        return classStatic.create(this, { user: sessionUser, branch, siteId }, body, params, {
            async final(user, userId) {
                ...
                const [result] = await mysql.execute(
                    query.user.registration.insert({
                        formId,
                        userId,
    ...
    static fetch = (
        ...
        if (!offline) {
            if (!sessionUser?.id)
                throw new Error('User Static Method Error [FETCH]: Session user not supplied');
            auth = false;
        }
    ...
    static mw = {
        login: async (req, res) => {
            ...
            try {
                ...
                let user = await User.fetch(
                    res.session,
                    { username },
                    { offline: true, auth: true, hideSensitive: false },
                );
                ...
                if (!matched) {
                    if (api) {
                        ...
                        if (fails <= loginAttempts && condition !== 'L') {
                            ...
                            update = await processData(update, {
                                query: User.config().query,
                                match: { id },
                                modifiedBy: 0,
                                branch,
                                siteId,
                        ...
                        return sendError.auth(req, res, 'Authentication failed: User not verified');
                }
                ...
                if (condition !== 'A') {
                    ...
                    if (api) {
                        ...
                        return sendError.auth(
                            req,
                            res,
                            `Authentication failed: ${conditions[condition]} user`,
                        );
                }
        ...
        session: async (req, res) => {
            try {
                ...
                const user = await User.fetch(
                    res.session,
                    { _id },
                    { offline: true, hideSensitive: false, hideEvents: false },
                );
                ...
                if (!verified) {
                    if (tokenKey !== providedToken)
                        return res.redirect(authUrl(res.session, _id, 'mismatch'));
                    else if (expired) {
        ...
        verify: async (req, res, next) => {
            ...
            try {
                    ...
                    if (api) sendError.auth(req, res, apiErrMsg);
                        ...
                        if (refer) {
                            const user = await User.fetch(
                                res.session,
                                { _id: refer },
                                { hideSensitive: false, hideEvents: false, offline: true },
                            );
                ...
                if (user.DS && user.location !== 'US')
                    return await reject(
                        'Verification failed: Incorrect status in current location',
                    );
                ...
                if (method !== 'POST' && !excUrl.includes(originalUrl)) await user.url(originalUrl);
        ...
        register: async (req, res) => {
            try {
                ...
                await mysql.execute(
                    query.user.registration.delete({ userId: User.matchIdHash(_id) }),
                );
        ...
        invite: async (req, res) => {
            try {
                ...
                const [result] = await mysql.execute(
                    query.user.registration.update(
                        {
...
class Role {
    ...
    static create = (session, body, params) =>
        classStatic.create(this, session, body, params, {
    ...
    static fetch = (
        session,
        filter,
        { hideRawId = false, sorts = Role.config().defSorts, limit, mode } = {},
    ) =>
        classStatic.fetch(
            this,
            session,
            ...
            },
            {
                ...
                prepare(batch, filter) {
                    const { id, _id, ids, _ids, category, name, location } = filter;
                    const single = !!id || !!_id;
...
class Token {
    ...
    static create = async (
        { userId, clientIp },
        { queryInst = query.session.token, UserSrc = User } = {},
    ) => {
    ...
    static fetch = async (
        ...
        const [rows] = await mysql.execute(
            queryInst.select(
                [[{ aes: ['token', tokenSecret] }, 'tokenKey'], 'verified', 'createdAt'],
                {
...
function authUrl(session, _id, status) {
    return `${addrBook.user}/authenticate?user=${_id}&branch=${btoa(session.branch)}&site=${btoa(session.siteId)}&status=${status}`;
}
