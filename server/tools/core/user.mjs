    ...
    if (single) {
      ...
      this.add = (target, ids = []) => {
        if (!this.session?.user?.id)
          throw new Error('User Constructor Method Error [ADD]: Session user not supplied');
      ...
      this.update = (body) => {
        const { session } = this;
        if (!session?.user?.id)
          throw new Error('User Constructor Method Error [UPDATE]: Session user not supplied');
            ...
            async final(user, body) {
              ...
              if (email && user.email !== email && !user.username) {
                if (user.events === undefined)
                  throw new Error('Failed to reinvite user due to missing event property');
      ...
      this.delete = (target, ids = []) => {
        if (!this.session?.user?.id)
          throw new Error('User Constructor Method Error [DELETE]: Session user not supplied');
        ...
        return classInstance.delete(this, new.target, target, ids, async () => {
          ...
          const [result] = await mysql.execute(
            query.user.main.update(update, { id: this.id || User.matchIdHash(this._id) }),
          );
      ...
      this.reset = async () => {
        if (!this.session?.user?.id)
          throw new Error('User Constructor Method Error [RESET]: Session user not supplied');
      ...
      this.invite = async () => {
        if (!this.session?.user?.id)
          throw new Error('User Constructor Method Error [INVITE]: Session user not supplied');
      ...
      this.settings = async (action = 'fetch', data = {}) => {
        ...
        let settings =
          (await mysql.execute(query.user.main.select('settings', { match })))[0][0].settings || {};
      ...
      try {
        ...
        if (condition !== 'A') {
          ...
          if (api) {
            ...
            return sendError.auth(req, res, `Authentication failed: ${conditions[condition]} user`);
        }
...
class Token {
  ...
  static fetch = async (
    ...
    const [rows] = await mysql.execute(
      queryInst.select([[{ aes: ['token', tokenSecret] }, 'tokenKey'], 'verified', 'createdAt'], {
