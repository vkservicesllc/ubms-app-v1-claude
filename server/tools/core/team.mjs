        ...
        if (single) {
            ...
            this.add = (target, bodyOrIds) => {
                if (!this.session?.user?.id)
                    throw new Error(
                        'Team Constructor Method Error [ADD]: Session user not supplied',
                    );
            ...
            this.update = (targetOrBody, body) => {
                if (!session?.user?.id)
                    throw new Error(
                        'Team Constructor Method Error [UPDATE]: Session user not supplied',
                    );
            ...
            this.delete = (target, matchOrIds) => {
                if (!this.session?.user?.id)
                    throw new Error(
                        'Team Constructor Method Error [DELETE]: Session user not supplied',
                    );
            ...
            this.settings = async (body) => {
                if (!this.session?.user?.id)
                    throw new Error(
                        'Team Constructor Method Error [SETTINGS]: Session user not supplied',
                    );
                ...
                if (body && typeof body === 'object') {
                    ...
                    settings.carrier.application.cdl = Number(
                        body?.carrier?.application?.cdl === 'on',
                    );
                    //? More to be added...
