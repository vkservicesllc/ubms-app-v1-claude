...
class Company {
  constructor(data = {}, { single = true, session, hideRawId = false, hideSensitive = true } = {}) {
    ...
    this.owner = data._ownerId
      ...
      : { _id: null };
    if (this.owner._id) this.owner.name = this.owner.fullName('FmLs');
    ...
    if (single) {
      ...
      this.update = (targetOrBody, body, match) =>
        classInstance.update(this, new.target, targetOrBody, body, match, {
          ...
          async final(company, body, target) {
            if (target !== 'main' || body.since === undefined || body.since === company.since)
              return;
            ...
            await renameFile(
              `${dir}/uploads/business/company/${company.id}/logo/`,
              `${company.since}.png`,
              `${body.since}.png`,
            );
      ...
      this.delete = (target, matchOrIds) =>
        classInstance.delete(this, new.target, target, matchOrIds, {
      ...
      { hideRawId, hideSensitive, sorts, limit, mode },
      {
        batch: [
          ...
          },
          {
            ...
            fields: [
              ['id', 'ownerId'],
              [Owner.hashId(), 'ownerId'],
              'personId',
              Individual.hashId('personId'),
              'parentId',
              Parent.hashId('parentId'),
            ],
          ...
          //! IMPORTANT: Filter this batch in external Category Class to avoid db/table coincidence in query
          {
            ...
            fields: [
              ['id', 'parentId'],
              [hash('id', algorithm.parent), '_parentId'],
            ],
            join: ['companyId', 'id'],
          },
          {
            ...
            fields: [
              ['id', 'carrierId'],
              [hash('id', algorithm.carrier), '_carrierId'],
            ],
            join: ['companyId', 'id'],
          },
          //! more to be added
        ...
        prepare(batch, filter) {
          const {
            ...
            closed,
            confirmed,
            active, // Combined when undefined
          } = filter;
          const single =
  ...
  static list = {
    category: {
      ...
      cst: {
        branch: 'construction',
        item: ['Builders', 'Builder'],
        group: 'Construction',
        path: ['builders', 'builder'],
      },
      hld: {
        branch: 'parent',
        item: ['Parents', 'Parent (Holding Company)'],
...
class Owner extends Individual {
    ...
    if (single) {
      ...
      this.add = async (target, body) => {
        if (!this?.session?.user?.id)
          throw new Error('Owner Constructor Method Error [ADD]: Session user not supplied');
      ...
      this.fetch = async (target, filter, params) => {
        if (!this?.session?.user?.id)
          throw new Error('Owner Constructor Method Error [FETCH]: Session user not supplied');

        const person = await Individual.fetch(this.session, {
          id: this.personId || Individual.matchIdHash(this._personId),
        });
        if (!person) throw new Error('Individual not found');
      ...
      this.update = async (targetOrBody, body, match = {}) => {
        if (!this?.session?.user?.id)
          throw new Error('Owner Constructor Method Error [UPDATE]: Session user not supplied');
      ...
      this.delete = () =>
        classInstance.delete(this, new.target, null, null, {
          extendLog(owner, log) {
            const reduntant = [
              'gender',
              'prefix',
  ...
  static create = async (session, body, params) => {
    if (!session?.user?.id)
      throw new Error('Owner Static Method Error [CREATE]: Session user not supplied');
  ...
  static fetch = (
    session,
    filter,
    { hideRawId = false, hideSensitive = true, sorts = Owner.config().defSorts, limit, mode } = {},
  ) =>
    classStatic.fetch(
      this,
      session,
      ...
      },
      {
        ...
        prepare(batch, filter) {
          ...
          const { id, _id, ssn, ids, _ids, gender, firstName, lastName, busName, type } = filter;
      ...
      { hideRawId, hideSensitive, sorts, limit, mode },
      {
        async prepare(batch, filter) {
          ...
          if (companyId || _companyId || companyIds || _companyIds)
            batch[0].match.id =
              companyId || companyIds || Company.matchIdHash(_companyId || _companyIds);
...
class RefSource {
  constructor(data = {}, { single = true, session, hideRawId = false, custom = {} }) {
    ...
    if (single) {
      ...
      this.add = (target, ids) => {
        if (!this.session?.user?.id)
          throw new Error('RefSource Constructor Method Error [ADD]: Session user not supplied');
      ...
      this.update = (body) => {
        if (!session?.user?.id)
          throw new Error('RefSource Constructor Method Error [UPDATE]: Session user not supplied');
      ...
      this.delete = (target, ids) => {
        if (!this.session?.user?.id)
          throw new Error('RefSource Constructor Method Error [DELETE]: Session user not supplied');
  ...
  static create = (session, body, params) => {
    if (!session?.user?.id)
      throw new Error('RefSource Static Method Error [CREATE]: Session user not supplied');
  ...
  static fetch = (
    session,
    filter,
    { hideRawId = false, offline = false, sorts = RefSource.config().defSorts, limit, mode } = {},
  ) => {
    if (!offline && !session?.user?.id)
      throw new Error('RefSource Static Method Error [FETCH]: Session user not supplied');
    ...
    return classStatic.fetch(
      ...
      { hideRawId, sorts, limit, mode },
      {
        ...
        prepare(batch, filter) {
          const { id, _id, name, ids, _ids } = filter;
