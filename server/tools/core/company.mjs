...
class Company {
    constructor(
        data = {},
        { single = true, session, hideRawId = false, hideSensitive = true } = {},
    ) {
        ...
        if (single) {
            ...
            this.add = (target, bodyOrIds) =>
                classInstance.add(this, new.target, target, bodyOrIds);
        ...
        if (single) {
            ...
            this.add = async (target, body) => {
                if (!this?.session?.user?.id)
                    throw new Error(
                        'Owner Constructor Method Error [ADD]: Session user not supplied',
                    );
            ...
            this.fetch = async (target, filter, params) => {
                if (!this?.session?.user?.id)
                    throw new Error(
                        'Owner Constructor Method Error [FETCH]: Session user not supplied',
                    );
            ...
            this.update = async (targetOrBody, body, match = {}) => {
                if (!this?.session?.user?.id)
                    throw new Error(
                        'Owner Constructor Method Error [UPDATE]: Session user not supplied',
                    );
                ...
                let { signature } = body;
                if (
                    [0, 1, false, true, null, '0', '1', 'false', 'true', 'null'].includes(signature)
                ) {
                    if (signature !== null) {
        ...
        classStatic.fetch(
            ...
            },
            {
                ...
                prepare(batch, filter) {
                    ...
                    const { id, _id, ssn, ids, _ids, gender, firstName, lastName, busName, type } =
                        filter;
                ...
                async prepare(batch, filter) {
                    ...
                    if (companyId || _companyId || companyIds || _companyIds)
                        batch[0].match.id =
                            companyId ||
                            companyIds ||
                            Company.matchIdHash(_companyId || _companyIds);
...
class RefSource {
    constructor(data = {}, { single = true, session, hideRawId = false, custom = {} }) {
        ...
        if (single) {
            ...
            this.add = (target, ids) => {
                if (!this.session?.user?.id)
                    throw new Error(
                        'RefSource Constructor Method Error [ADD]: Session user not supplied',
                    );
            ...
            this.update = (body) => {
                if (!session?.user?.id)
                    throw new Error(
                        'RefSource Constructor Method Error [UPDATE]: Session user not supplied',
                    );
            ...
            this.delete = (target, ids) => {
                if (!this.session?.user?.id)
                    throw new Error(
                        'RefSource Constructor Method Error [DELETE]: Session user not supplied',
                    );
    ...
    static fetch = (
        ...
        filter,
        {
            hideRawId = false,
            offline = false,
            sorts = RefSource.config().defSorts,
            limit,
            mode,
        } = {},
    ) => {
