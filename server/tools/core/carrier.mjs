        ...
        classStatic.fetch(
            ...
            { hideRawId, hideSensitive, sorts, limit, mode },
            {
                async prepare(batch, filter) {
                    ...
                    //! IMPORTANT
                    batch = batch.filter(
                        (item) => item.table !== query.carrier.main.table && item.db !== db.carrier,
                    );
                    ...
                    Object.keys(stateTax).forEach(
                        (state) => (batch[idx + 2].match = { [state]: stateTax[state] }),
                    );
                    ...
                    if (id || _id || ids || _ids) {
                        if (id) batch[idx].match.id = id;
                            ...
                            batch[idx].match.id = Carrier.matchIdHash(_id || _ids);
                    }
