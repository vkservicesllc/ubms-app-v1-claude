...
export const initialProgress = async (inst, step, body) => {
  ...
  switch (step) {
    ...
    case 'residence': //* Driver Application
      {
        ...
        const { since, currentSince, enough, livedAbroad, address1, address2, city, state, zip } =
          address;
    ...
    case 'prev-employment': //* Driver Application
      {
        ...
        if (!body.prevEmployed)
          await mysql.execute(query.driver_employment.main.delete({ driverId: driver.id }));
        await inst.update(body);
        await driver.update('appDef', { cache });
      }
      break;
    ...
    case 'business':
      {
        let { activeLLC } = body;
        const { inactiveLLC, busName, state, ein, mmt, type, make, model, year, length, trailer } =
          body;
