...
import { initialProgress, rehireProgress } from './driver/application.mjs';
...
class Driver extends Individual {
    constructor(data = {}, { single = true, session, hideRawId = false, hideSensitive = true }) {
        ...
        const { id, _id, personId, _personId, blackListed, complete, prevCountry, expDate, cache } =
            data;
        const properties = {
            first: { _id, _personId },
            last: {
        ...
        return classStatic.fetch(
            ...
            { hideRawId, sorts, limit, mode },
            {
                ...
                prepare(batch, filter, session) {
                    ...
                    const { id, _id, personId, _personId, ssn, blackListed, teamId, _teamId } =
                        filter;
                    ...
                    if (!personId && _personId)
                        batch[0].match.personId = Individual.matchIdHash(_personId);
...
class Application {
    constructor(data = {}, { single = true, session, hideRawId = false, hideSensitive = true }) {
        ...
        if (data.leadLastName || data.leadSsn) {
            ...
            if (this.lead.firstName && this.lead.lastName) this.lead = new Person(this.lead);
        ...
        if (single) {
            ...
            this.register = async (body) => {
                ...
                if (!person) {
                    person = (
                        await Individual.create(this.session, {
                            ssn,
                            dob,
                            gender,
                            prefix,
            ...
            this.fetch = async (target, filter = {}, { hideRawId = false } = {}) => {
                ...
                const driver = await Driver.fetch(this.session, {
                    id: this.driverId || Driver.matchIdHash(this._driverId),
                });
                            ...
                            batch = [
                                {
                                    ...
                                    match: {
                                        appId: this.id || Application.matchIdHash(this._id),
                                        since,
                                    },
                                    sort: { desc: 'since' },
                                },
            ...
            this.delete = (target, match = {}) =>
                classInstance.delete(this, new.target, target, match);
            ...
            this.submit = async () => {
                if (this.condition === 'p' && this.step === 12) {
                    ...
                    for (const employment of employments) {
                        ...
                        await mysql.execute(
                            query.driver_employment.verifications.update(
                                { verify },
                                { emplId, appId },
                            ),
                        );
            ...
            this.identity = async () => {
                ...
                const individual = await Individual.fetch(
                    this.session,
                    { _id: this._personId },
                    this.config,
                );
                ...
                props.forEach(
                    (prop) =>
                        (mismatch[prop] = !!individual[prop] && this[prop] !== individual[prop]),
                );
    ...
    static invite = async (session, data, formId) => {
        ...
        if (!team && (teamId || _teamId))
            team = await Team.fetch(session, { id: teamId, _id: _teamId });
        ...
        const mailOpts = {
            ...
            html: `<div style="font-family: Arial, Helvetica, sans-serif;">
                Dear Friend,<br/>
                ${
                    companyName ? `${companyName} invites you` : 'You are invited'
    ...
    static create = (session, body, params) =>
        classStatic.create(this, session, body, params, {
            async split(body) {
                ...
                if (!team && _teamId && _teamId !== 'global')
                    team = await Team.fetch(session, { _id: _teamId }, { offline: true });
                if (team) teamId = team.id;
                if (user && selfAssign) userId = user.id;
                else if (_userSimpleId) {
                    const user = await User.fetch(
                        session,
                        { _simpleId: _userSimpleId },
                        { offline: true },
                    );
                ...
                if (!driver) {
                    ...
                    body.lead = !person
                        ? { ssn: processAES('ssn', ssn), dob, gender }
                        : { personId: person.id };
                ...
                const person = await Individual.fetch(
                    session,
                    { id: personId },
                    { hideSensitive: false },
                );
    ...
    static fetch = (
        ...
        filter,
        {
            hideRawId = false,
            hideSensitive = true,
            sorts = Application.config().defSorts,
            limit,
            mode,
        } = {},
    ) =>
            ...
            { hideRawId, hideSensitive, sorts, limit, mode },
            {
                ...
                prepare(batch, filter, session) {
                    ...
                    if (!carrierId && carrierId !== null)
                        match.carrierId = Carrier.matchIdHash(_carrierId);
    ...
    static assigned = async (session) => {
        if (!session.user.id)
            throw new Error(
                'Application Static Method Error [ASSIGNED]: Session user not supplied',
            );
        const { team } = session;
    ...
    static chart = async (session, filter = {}) => {
        ...
        if (!teamId && !teamId) batch[1].match = { scoped: [false, null] };
...
class Employment {
        ...
        if (single) {
            ...
            this.add = async (target, body) => {
                if (!this.session?.user?.id)
                    throw new Error(
                        'Employment Constructor Method Error [ADD]: Session user not supplied',
                    );
                    ...
                    case 'attempts':
                        {
                            ...
                            const [result] = await mysql.execute(
                                query.driver_employment.attempts.insert(body),
                            );
            ...
            this.fetch = async (target, filter = {}, { hideRawId = false } = {}) => {
                if (!this.session?.user?.id)
                    throw new Error(
                        'Employment Constructor Method Error [FETCH]: Session user not supplied',
                    );
                    ...
                    case 'attempts':
                        {
                            ...
                            if (!hideRawId)
                                batch[0].fields.unshift('emplId', 'appId', 'inquiredBy');
    ...
    static fetch = (
        ...
        filter = {},
        {
            hideRawId = false,
            hideSensitive = true,
            sorts = Employment.config().defSorts,
            limit,
            mode,
        } = {},
    ) => {
