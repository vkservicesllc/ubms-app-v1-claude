    ...
    return classStatic.fetch(
      ...
      { hideRawId, sorts, limit, mode },
      {
        ...
        prepare(batch, filter, session) {
          ...
          const { id, _id, personId, _personId, ssn, blackListed, teamId, _teamId } = filter;
...
class Application {
  constructor(data = {}, { single = true, session, hideRawId = false, hideSensitive = true }) {
    ...
    if (single) {
      ...
      this.submit = async () => {
        if (this.condition === 'p' && this.step === 12) {
          ...
          for (const employment of employments) {
            ...
            await mysql.execute(
              query.driver_employment.verifications.update({ verify }, { emplId, appId }),
            );
          }
      ...
      this.identity = async () => {
        ...
        props.forEach(
          (prop) => (mismatch[prop] = !!individual[prop] && this[prop] !== individual[prop]),
        );
  ...
  static invite = async (session, data, formId) => {
    ...
    const { cdlRole, vhlCode, carrierId, _carrierId, teamId, _teamId, _userSimpleId, selfAssign } =
      data;
    ...
    classStatic.fetch(
      ...
      { hideRawId, hideSensitive, sorts, limit, mode },
      {
        batch: [
          ...
          {
            table: query.driver.mecs.table,
            fields: [['expiresOn', 'mecExpiresOn'], ['issuedOn', 'mecIssuedOn'], 'nrcme'],
            join: ['id', 'mecId'],
  ...
  static assigned = async (session) => {
    if (!session.user.id)
      throw new Error('Application Static Method Error [ASSIGNED]: Session user not supplied');
    const { team } = session;
    ...
    if (single) {
      ...
      this.add = async (target, body) => {
        if (!this.session?.user?.id)
          throw new Error('Employment Constructor Method Error [ADD]: Session user not supplied');
      ...
      this.fetch = async (target, filter = {}, { hideRawId = false } = {}) => {
        if (!this.session?.user?.id)
          throw new Error('Employment Constructor Method Error [FETCH]: Session user not supplied');
