...
class Individual extends Person {
  constructor(data = {}, { single = true, session, hideRawId = false, hideSensitive = true }) {
    ...
    if (single) {
      ...
      this.log = (params) =>
        classInstance.log(this, new.target, params, [...classInstance.logFields, 'createdIn']);
    }
  }
  ...
  static create = (session, body, params) =>
    classStatic.create(this, session, body, params, {
      ...
      split(body) {
        const { dob, gender, ssn, prefix, firstName, middleName, lastName, suffix, alias } = body;
  ...
  static fetch = (
    ...
    filter,
    {
      hideRawId = false,
      hideSensitive = true,
      sorts = Individual.config().defSorts,
      limit,
      mode = 'data',
    } = {},
  ) => {
      ...
      },
      {
        ...
        batch: [
          ...
          },
          {
            ...
            fields: [['since', 'legalSince'], 'status', ['expiresOn', 'statusExpiredOn']],
            join,
          },
          {
          ...
          },
          {
            ...
            fields: [['since', 'addrSince'], 'address1', 'address2', 'city', 'state', 'zip'],
            join,
          },
          {
          ...
          },
          {
            ...
            join: ['personId', 'id', 0, ['since', 'addrSince', query.person.identifications.table]],
          },
          //? need it?
          {
            db: db.business,
        ...
        prepare(batch, filter) {
          const { id, _id, ssn, ids, _ids, gender, firstName, lastName } = filter;
