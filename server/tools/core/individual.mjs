...
class Individual extends Person {
    constructor(data = {}, { single = true, session, hideRawId = false, hideSensitive = true }) {
        ...
        if (single) {
            ...
            this.update = (targetOrBody, body, match) =>
                classInstance.update(this, new.target, targetOrBody, body, match, {
                    ...
                    async final(person, body, target) {
                        if (target !== 'main' || body.dob === undefined || body.dob === person.dob)
                            return;
                        ...
                        for (const target of keys)
                            await mysql.execute(
                                query.person[target].update({ since: body.dob }, match),
                            );
                    },
                });
            ...
            this.log = (params) =>
                classInstance.log(this, new.target, params, [
    ...
    static create = (session, body, params) =>
        classStatic.create(this, session, body, params, {
            async find(body, hideRawId) {
                ...
                if (ssn) {
                    ...
                    if (data && data.dob !== dob)
                        throw new Error('SSN/DOB mismatch (SSN recognized)');
                }
            ...
            split(body) {
                const { dob, gender, ssn, prefix, firstName, middleName, lastName, suffix, alias } =
                    body;
